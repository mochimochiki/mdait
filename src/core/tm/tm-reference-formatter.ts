 ...
 * 一致度を百分率の文字列にする。
 *
 * **100% と読めるのは本当に完全一致のときだけ**にする。0.996 を四捨五入して 100% と出すと、
 * 受け取る側は「同じ文だ」と読むが実際は違う文なので、下は切り捨てて 99% で止める。
 *
 * 数として読めない値（NaN・Infinity）は 0% にする。範囲外の有限値（1.5 など）は端に丸めるが、
 * **そもそも数になっていない値を 100% に倒してはいけない** — 計算が壊れたときに、
 * いちばん強い合図（同じ文だ）が出てしまう。弱いほうへ倒すのが安全な側である。
 */
function formatMatchRate(similarity: number): number {
	...
	return Math.min(99, Math.floor(similarity * 100));
}
