 ...
 * 16/20、2000件へ50件ずつで 18/20）。1行なら競合の範囲が1行に収まり、両陣営の値をそのまま
 * 切り出せる。重複した TU は読み込みが畳む。
 *
 * かつては `.gitattributes` の `merge=union` に両方の行を残させていたが、union は SVN に
 * 無く、効いた先で黙って片方を捨てていたので撤去した（ADR-260911-01）。1行に収める工夫の
 * ほうは、競合を解くときに両側を切り出しやすいので残してある。
 */
