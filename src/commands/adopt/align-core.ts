 ...
 * @module commands/adopt/align-core
 */
...
export async function alignMatchResult(
	...
	if (maxUnits > 0 && Math.max(sourceUnits.length, targetUnits.length) > maxUnits) {
		...
		logger.info("adopt", "Align skipped: too many units", {
	...
	if (validation.rejected.length > 0) {
		logger.info("adopt", "Align rejected some corrections", {
	...
	logger.info("adopt", "Align applied corrections", {
		candidatePairs,
		accepted: validation.accepted.length,
