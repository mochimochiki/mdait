 ...
 * @module commands/adopt/section-aligner
 */
...
export class SectionAligner {
	...
	async align(request: SectionAlignRequest, token?: vscode.CancellationToken): Promise<SectionAlignResult> {
		const promptParts = this.getPromptParts(PromptIds.ADOPT_ALIGN, {
		...
		if (refs.length > this.limits.maxNeedBodies || this.limits.maxRounds < 2) {
			logger.info("adopt", "Align falling back (needBodies over limit or single-round)", {
	...
	private async requestRound(
		...
		logger.warn("adopt", "Align response invalid after retries", {
