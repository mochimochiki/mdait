...
export class PairVerifier {
	...
	async verifyBatch(
		...
		const promptParts = this.getPromptParts(PromptIds.AI_SYNC_VERIFY_PAIRING_BATCH, {
			sourceLang: request.sourceLang,
			targetLang: request.targetLang,
		...
		for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
			...
			if (attempt > 0) {
				Logger.getInstance().warn("aiSync", "Batch pairing verification retry", {
					attempt: attempt + 1,
					maxRetries: this.maxRetries + 1,
					reason: lastError.message,
		...
		Logger.getInstance().error("aiSync", "Batch pairing verification incomplete after all retry attempts", {
			totalAttempts: this.maxRetries + 1,
			lastError: lastError ? formatError(lastError) : "No error details available",
			pairCount: request.pairs.length,
