...
export class TranslationContext {
	...
	/**
	 * 前回の訳文。**原文が改訂された（need:revise）ときだけ入れる。**
	 *
	 * 変更不要な部分は既訳を尊重し、変更が必要な箇所のみを変更するための参考情報であり、
	 * 差分パッチ翻訳ではパッチを当てる土台そのものになる。
	 * 初回翻訳のユニットは中身が原文の丸写しなので、ここに入れると
	 * 「原文が改訂された」という嘘の枠つきで原文を送り返すことになる。
	 */
