...
import {
	type AiSyncOutcome,
	buildAiSyncNextActions,
	formatSyncLine,
	generateAiSyncReportContent,
} from "../../../../commands/ai-sync/ai-sync-result";
...
function outcome(overrides: Partial<AiSyncOutcome> = {}): AiSyncOutcome {
	return { sync: syncResult(), review: [], dryRun: false, aborted: false, ...overrides };
}

suite("generateAiSyncReportContent（合成レポート・純関数）", () => {
	test("sync 段のサマリとレビュー段のサマリを両方含む", () => {
		const report = generateAiSyncReportContent(
			outcome({
	...
	test("aborted のときは sync が走らなかった旨を出す", () => {
		const report = generateAiSyncReportContent(outcome({ sync: undefined, aborted: true }));
		assert.ok(report.includes("Sync did not run"));
	});

	test("dryRun のときはマーカー不変の注記を出す", () => {
		const report = generateAiSyncReportContent(outcome({ dryRun: true, review: [reviewFile([unit("kept", "match")])] }));
		assert.ok(report.includes("dry run"));
	});

	test("レビュー0件でもレポートは生成される（冪等 no-op）", () => {
		const report = generateAiSyncReportContent(outcome());
		assert.ok(report.includes("verified: 0"));
	});
});

suite("buildAiSyncNextActions（合成 nextActions・純関数）", () => {
	test("aborted のときは設定確認を促す1件のみ", () => {
		const actions = buildAiSyncNextActions(outcome({ sync: undefined, aborted: true }));
		assert.strictEqual(actions.length, 1);
		assert.ok(actions[0].includes("Sync did not run"));
	});

	test("mismatch があれば構造修正→再同期を促す", () => {
		const actions = buildAiSyncNextActions(outcome({ review: [reviewFile([unit("escalated", "mismatch")])] }));
		assert.ok(actions.some((a) => a.includes("mis-paired") && a.includes("mdait_aiSync")));
	});

	test("approved があれば mdait_tm commit を促す", () => {
		const actions = buildAiSyncNextActions(outcome({ review: [reviewFile([unit("approved", "match")])] }));
		assert.ok(actions.some((a) => a.includes("auto-approved") && a.includes('mdait_tm (action:"commit")')));
	});

	test("dryRun かつ検証ありなら再実行を促す", () => {
		const actions = buildAiSyncNextActions(outcome({ dryRun: true, review: [reviewFile([unit("kept", "match")])] }));
		assert.ok(actions.some((a) => a.includes("dry run") && a.includes("without dryRun")));
	});

	test("何も残っていなければ clean メッセージ（冪等 no-op）", () => {
		const actions = buildAiSyncNextActions(outcome());
		assert.strictEqual(actions.length, 1);
