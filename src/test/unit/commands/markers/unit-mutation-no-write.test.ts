...
// 原文へそのまま焼き付いていた。
//
// 止める基準はモードでも原文/訳文でもなく「中身が変わったか」に置く。
// deleteUnit は external でも本文から章そのものを消すので書き込みが要る。
// モードを基準にすると、この経路が壊れる。
 ...
 *
 * **この検査が守れるのは、パーサーが書き出す姿と元の本文が一致する原稿だけである。**
 * 改行コードが CRLF・章のあいだの空行が2行以上・末尾に改行が無い、といった原稿は
 * 書き出しの整形で本文そのものが変わるため、ここでの「中身の比較」では止まらない
 * （実測済み。原文を触らない保証をそこまで広げるのは別の手当てになる）。
 */
