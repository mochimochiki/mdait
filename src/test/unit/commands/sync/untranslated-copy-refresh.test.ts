 ...
 * ユニットの本文は古い原文の丸写しのまま**だった。その結果 `need:translate` のまま
 * `hash`（訳文の中身）と `from`（原文の中身）が食い違い、
 * `MdaitMarker.hasUnconfirmedEdit()` がその食い違いを手編集の証拠として読むため、
 * **人が一度も触っていないユニットが「編集済み — まだ完了にしていません」と名乗る**。
 * ツリーは同じユニットを「未翻訳」と出すので、2つのサーフェスが食い違う。しかも案内に
 * 従って「翻訳済みにする」を押すと、原文のままの本文が完成品として確定される。
 *
 * 直し方は表示ではなく状態のほうで、**「未訳の訳文は今の原文の丸写しである」という
 * 不変条件を sync が保つ**ようにした。写し直してよい根拠は「その訳文に人の仕事が
 * 入っていない」ことだけなので、丸写しであることを確かめたユニットにしか触らない。
 */
		...
		test("原文が変わると、未訳の丸写しも新しい原文へ写し直される", async () => {
			...
			for (const marker of targetMarkers()) {
				...
				assert.strictEqual(
					marker.hash,
					marker.from,
					"hash と from が食い違うと、触っていないユニットが「編集済み」を名乗る",
				);
			}
		});
