...
import { HELD_ORDER_BASE, type UnitStateEntry } from "../../../../core/unit-state/unit-state-store";
...
suite("失った翻訳の状態を数える", () => {
	test("訳し直して hash が変わっただけなら、失ったと数えない", () => {
		...
		assert.strictEqual(countLostStateEntries(previous, units), 0, "章はまだそこに在る（from が同じ）");
	});
	...
	test("章そのものが消えたら、失ったと数える", () => {
		...
		assert.strictEqual(countLostStateEntries(previous, units), 1);
	});

	test("位置が変わっただけ（hash が生きている）なら数えない", () => {
		const previous = [entry({ order: 0, hash: "h1", from: "s1" })];
		const units = [unit("h9", "s9"), unit("h1", "s1")];

		assert.strictEqual(countLostStateEntries(previous, units), 0);
	});

	test("守るべき状態が無い行（from も need も空）は数えない", () => {
		const previous = [entry({ hash: "h1", from: "", need: "" })];
		assert.strictEqual(countLostStateEntries(previous, [unit("h2", "s2")]), 0);
	});

	test("保留席の行は上書きされないので数えない", () => {
		const previous = [entry({ order: HELD_ORDER_BASE, hash: "h1", from: "s1", need: "translate" })];
		assert.strictEqual(countLostStateEntries(previous, [unit("h2", "s2")]), 0);
	});

	test("末尾の余った行は、刈り取り／保留の判断が別にあるので数えない", () => {
		const previous = [entry({ order: 5, hash: "h1", from: "s1", need: "translate" })];
		assert.strictEqual(countLostStateEntries(previous, [unit("h2", "s2")]), 0);
	});
	...
	test("from を持たない独立ユニットは、これまでどおり hash で見る", () => {
		...
		assert.strictEqual(countLostStateEntries(previous, [unit("h1")]), 0, "同じ hash が居れば残っている");
		assert.strictEqual(countLostStateEntries(previous, [unit("h2", "s2")]), 1, "居なければ失われている");
	});
});
