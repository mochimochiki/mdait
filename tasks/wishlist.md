...
need:reviewのときのUIを改善する
- CodeLens上にマークが目立たないので、最も左側（原文より左）にレビューが必要であることを示すアイコンを表示する
- TranslationSummaryHoverProvider「翻訳完了」と出ているが、実際にはレビューが必要な状態なので、文言を「要レビュー」などに変更する
