...
- **実装**: ターゲットファイルからパースしたユニットの`content`フィールドには、翻訳前の状態（＝前回の訳文）が含まれている
- **詳細**: 
  - 翻訳処理は`need:translate`フラグが付いたユニットに対して実行される
  - 翻訳前の時点では、ユニットの`content`はまだ前回の訳文のまま
  - 翻訳後に`unit.content = translatedText`で新しい訳文に更新される
  - したがって、`unit.marker?.from`が存在する場合（＝原文が改訂された場合）、`unit.content`を前回訳文として使用できる
...
- [x] 前回訳文の取得ロジック実装（trans-command.ts内で実装）
  - `unit.marker?.from`が存在する場合、`unit.content`を前回訳文として使用
  - シンプルな実装でファイル再読み込み不要
...
- [x] テスト実装
  - [x] translation-checker のテスト（9つのテストケース）
  - previous-translation-finderは統合的にテストされる
  
- [x] コードレビューと品質チェック
  - [x] コードレビュー実施（1件のコメント、論理的に問題なし）
  - [x] セキュリティチェック実施（脆弱性なし）
  
## 実装完了

すべての実装が完了しました。以下の機能が正常に動作します：

1. **前回訳文の参照**: 原文改訂時に、翻訳前のユニットの`content`（＝前回の訳文）を翻訳プロンプトに含める
2. **変更箇所の最小化**: AIに対して、変更不要な部分は既訳を尊重するよう指示
3. **翻訳品質チェック**: 数値、リスト項目数、コードブロック数の不一致を自動検出
4. **need:reviewステータス**: 確認推奨箇所がある場合、`need:review`を設定
5. **ツールチップ表示**: Hoverで「要確認」と表示し、レビュー推奨理由を提示

## 実装の改善

レビューコメントを受けて、前回訳文の取得方法を簡素化：
- 当初は別ファイルからユニットを検索する複雑な実装だったが、翻訳前のユニット自体に前回訳文が含まれていることに気付き、シンプルな実装に変更
- `PreviousTranslationFinder`クラスを削除し、`unit.content`を直接使用するように修正

## 動作確認（推奨）

実際のMarkdownファイルで以下を確認することを推奨します：
- 原文を修正してsyncを実行
- 翻訳を実行して前回訳文が参照されること
- 数値やリスト項目に変更を加えた場合、need:reviewが設定されること
- Hoverで「要確認」と理由が表示されること
