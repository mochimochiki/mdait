...
- `--profile=mdait-debug` でのデバッグ起動でvscode-lm APIが使えることは確認済み（`--profile-temp`から変更。AI同意ダイアログの永続化のため）
...
- [x] m.coderによる実装（第2マイルストーン: ログキャプチャ）
  - [x] LoggerにonLogコールバック機構追加（addLogListener → Disposable）
  - [x] DebugCommandHandlerでログキャプチャ実装（capturedLogs + finally dispose）
- [x] launch.jsonを`--profile=mdait-debug`に変更（AI同意ダイアログの永続化）
- [x] IPC経由でのsync・trans統合テスト実施・ログキャプチャ動作確認
...
第2マイルストーンとしてLoggerにonLogリスナー機構（`addLogListener` → `Disposable`）を追加し、DebugCommandHandlerがコマンド実行中のログを`result.json`の`logs`フィールドにキャプチャするようになった。また`--profile-temp`を`--profile=mdait-debug`に変更し、vscode-lm同意ダイアログがプロファイルに記憶されるようにした。IPC経由でのsync・trans統合テストで正常動作とログキャプチャを確認済み（283テストパス）。
