...
- [ ] Markdownパーサーとの統合（`need:translate`ユニット特定）
- [ ] `TranslationContext`への具体的情報設定ロジック
...
### 残課題
- **Markdownパース**: ファイル全体処理から`need:translate`ユニット単位処理への移行
- **`TranslationContext`**: `surroundingText`、`glossary`設定ロジック未実装
- **実AIプロバイダ**: OpenAI、Anthropic等の実際のAPI接続
- **エラー分類**: 詳細なエラーハンドリングと分類処理

### テスト観点
- **ユニット翻訳**: `need:translate`フラグ識別と翻訳処理
- **ハッシュ更新**: 翻訳後のハッシュ再計算とフラグ除去
- **設定反映**: コードブロックスキップ設定の動作確認
- **エラーハンドリング**: ファイル読み書き、API呼び出しエラー処理
- **ストリーミング**: AI応答のストリーミング処理

## 7. 次のステップ

### 最優先（P0）
1. **Markdownパーサー統合**: `need:translate`ユニット特定とユニット単位処理への移行
2. **実AIプロバイダ実装**: OpenAI API等の実際のAIサービス接続

### 高優先（P1）
3. **`TranslationContext`詳細設定**: 前後テキスト、用語集設定ロジック実装
4. **言語ペア設定**: `mdait.transPairs`に基づく翻訳言語決定機能
5. **エラーハンドリング強化**: 詳細なエラー分類と適切な処理

### 中優先（P2）
6. **進捗表示機能**: 大量ファイル処理時のユーザーフィードバック
7. **ユニットテスト**: 各モジュールの単体テスト実装
8. **統合テスト**: エンドツーエンドの翻訳フロー検証
