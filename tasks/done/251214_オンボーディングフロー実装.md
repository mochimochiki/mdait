...
### Phase 8: 初回同期ボタン実装（追加改善）
- [x] `StatusItemTree.isEmpty()`メソッド実装
- [x] `mdaitHasStatus`コンテキスト変数管理（extension.ts）
- [x] ボタンの`when`条件を更新（初回同期/通常同期の分離）
- [x] `setup-command`でコンテキスト変数更新
- [x] l10n対応（初回同期ボタンのラベル追加）
- [x] テスト実行（36テスト全パス）
