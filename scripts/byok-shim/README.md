...
作業場所は空のディレクトリ）。mdait を知っている翻訳者になってしまうと、素の振る舞いが見えなくなるからである。

同時に走る数は `--agent-concurrency` で抑える。既定は4。
...
**翻訳の質を測ることはできない。** 翻訳役に据える `claude` は mdait の中身を知っているので、
素の LLM より賢く振る舞ってしまう。この道具は形の検証と、わざと壊すためのものである。
プロンプトの善し悪しを比べる用途には使わない。
