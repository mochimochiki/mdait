...
mdait が AI へ投げる仕事は翻訳だけではない。**用語を拾う・訳語を埋める・文の対を作る**の3つは
答えの形が違う（JSON の配列や対応表）ので、`lib/echo-answers.mjs` が指示文（system）の言い回しから
仕事の種類を見分けて、その形で答える。見分けが付かない仕事はこれまでどおり訳文を返す。

作り話は混ぜない。**指示文と依頼文に実際に書かれている文字列だけ**を使って組み立てる — mdait 側に
「用語は原文に出てくること」「文の対は原文・訳文の一部そのままであること」という検査があり、
作り話では結局0件になるからである。おかげでスイープ（P9・P10）は用語集と翻訳メモリが**実際に
増えること**まで見ている（ADR-260907-02）。
