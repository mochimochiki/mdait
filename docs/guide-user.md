...
| レビュー待ちの訳を採用せず訳し直す | CodeLens **要翻訳にする** → **✨翻訳** |
| 確認待ちを順に片づける | ビューの **要対応** ノード（クリックで訳文と原文を並べて表示） |
...
| `review` | 人の確認待ち | 訳文を読み、問題なければ **レビュー完了**。採用しないなら **要翻訳にする**（翻訳待ちに戻す。AI は呼ばない） |
| `verify-deletion` | 原文側が削除された | **保持** か **ユニットを削除** を選ぶ |
| `isolate` | 翻訳対象外として凍結中 | 何もしない |

`need` が付いていないユニットは同期済み・作業不要。訳文側で `from:` を持たないマーカーは**独立ユニット**、つまり原文に対応相手のない独自セクションで、常に保持され翻訳も削除もされない。見出しの右に薄く `原文なし` と出るのがその印で、ボタンは何も出ない（するべき操作が無いため）。マウスを乗せると理由が出る。

「凍結」（`isolate`）とは別物である。凍結は**原文の章がまだ在って**、その更新を流さないだけなので、いつでも解除して追従を再開できる。独立ユニットは結びつく原文そのものが無く、元には戻せない。

`review` は、AI が訳文の構造を原文と揃えられなかったとき、既存訳を取り込んだとき、マーカーのない訳文が見つかったとき（取り込みを経ていない同期でも同じ。原文をそのまま写しただけの訳文は `translate` になる）に付く。内容に問題がなければ **レビュー完了** で外すのが最短。訳が駄目なら **要翻訳にする** で翻訳待ちに戻し、あらためて **✨翻訳** する。

## 訳文を確認する

原文のマーカーにカーソルを乗せると、現在の訳文とステータスがポップアップする。腰を据えて見比べるときは CodeLens の **原文** / **訳文** で対応ユニットへジャンプする。

確認待ちがたまったら、ビューの **要対応** ノードを使う。項目をクリックすると訳文と原文が左右に並んで開くので、その場で **レビュー完了** か **要翻訳にする** を押す。**レビュー完了**（要対応ノードの項目にある **レビュー済みにする** も同じ）を押すと、残っている次の要対応へそのまま進む（残りが無くなるとステータスバーに「要対応: すべて片づきました」と出る）。**要翻訳にする** を押したときはその場に留まるので、続けて **✨翻訳** を押すか、**次の要対応へ**（`Ctrl+Alt+N`）で次へ進む。

ユニット数が多いときは、行の **✨AI翻訳レビュー** で AI に一次判定させられる。忠実な訳と判定されたユニットは `need:review` が自動で外れ、誤ったペアや訳抜けの疑いだけが残る。要対応ノードの **✨AI翻訳レビュー** は、いま残っている確認待ちを全部まとめてかける。同期の完了通知にも、翻訳待ちが無く確認待ちだけが残っているときは **✨AIレビュー** ボタンが出る。
