...
> **上位設計**: [commands.md](commands.md)、[design.md](design.md)「term - 用語管理」参照

## 役割

翻訳品質向上のための用語管理機能を提供します。AIを使用して原文から重要用語を検出し（`term.detect`）、既存の翻訳から用語訳を抽出して用語集を構築・管理します（`term.expand`）。

**設計意図**: 用語の一貫性は翻訳品質の要です。自動検出と自動展開により、用語集のメンテナンス負担を軽減します。
...
**コマンド**: `mdait.status.openTerm`

**機能**: `.mdait/`配下の用語集ファイル（`terms.csv`または`terms.yaml`）をVSCodeエディタで開き、用語集の内容を確認・編集可能にする。

**UI起点**: ステータスビューのナビゲーションボタン（`$(repo)`アイコン）

**実装**: `src/commands/term/command-open.ts`

**エラーハンドリング**:
- ワークスペースが見つからない場合: エラーメッセージ表示
- 用語集ファイルが存在しない場合: 情報メッセージ表示（まだ用語が登録されていない状態）

**注意事項**: 用語集の手動編集は可能ですが、ファイル形式（CSV/YAML）の構造を維持する必要があります。`term.detect`や`term.expand`コマンド経由での管理を基本とし、微調整のみ手動編集することを推奨します。

**設計意図**: 用語集の内容を直接確認・編集できることで、用語管理の柔軟性が向上します。
