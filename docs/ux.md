...
4. **「次の要対応へ」**: `mdait.needsAttention.next` を CodeLens・コンテキストメニュー・キーバインドの3導線で提供。自動ジャンプはしない（UX-P5）。※CodeLens の導線はその後外し、移動先は訳文と原文を並べた対訳表示に変えた（ADR-260912-02）。※さらにその後、「レビュー完了」（CodeLens・ツリー）の直後だけは次の要対応へ自動で進むと決め直した（ADR-260912-03。裁定→移動を1操作にしないと連続裁定にならない）。
