...
| **Decoration**（ユニット右のインライン薄字） | そのユニットの**状態を一言で**（`翻訳完了 : 1.2秒` / `編集済み — まだ完了にしていません`） | 操作。長文。折り返すほどの情報 |
...
- 状態 → Decoration に `編集済み — まだ完了にしていません`（編集されたユニットにだけ出す）
- 解説 → Hover に「完了はあなたが決める。仕上がったら『翻訳済みにする』を押す」
