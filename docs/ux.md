...
- **AI を使う操作には ✨ を付ける**（UX-P4）。付いていない操作は AI を呼ばない。**逆も真** — AI を呼ばない操作に ✨ を付けない（実例: `✨TM最適化` は純粋な重み計算で AI を呼ばないのに ✨ が付き、コストの予測を裏切っていた）
...
| F-8 | `✨TM最適化` が非AIなのに ✨ 付き。コストの予測を裏切る | 低 | 🔜 UX-R5（UI から廃止） |
...
1. **消す・畳む**（F-7〜F-9）
   ...
   - `✨TM最適化` を UI から廃止し、TM 登録の後段で自動実行する
