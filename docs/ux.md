...
| F-2 | `hasUnconfirmedEdit()` が `marker.hash`（訳文テキストのハッシュ）と `revise@X` の X（**旧原文ハッシュ**）を比較しており、別種の値なので実質つねに真。訳文に一度も触れていない revise ユニットにも `編集済み — まだ完了にしていません` が出る | 重大（バグ） | 🔜 UX-R5 |
...
- 訳文に触れていない `need:revise` ユニットに「編集済み」が出ないこと（単体テスト）
