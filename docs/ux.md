...
| B-8 | 裁定後に次の項目へ進む導線がなく、ツリー→ジャンプ→裁定→ツリーの往復が残る（B-3で一覧は得たがキューとして未完成） | 中 | ✅ `mdait.needsAttention.next`（次の要対応へ）を3導線で追加（ADR-260724-01）。※その後 CodeLens の「Next」は外した（ADR-260912-07。マーカー行に並べるのはそのユニットへの操作だけ）。いまの導線は要対応ノードのインライン・パレット・キーバインドで、移動先は訳文と原文を並べた対訳表示（`mdait.openPair`） |
...
4. **「次の要対応へ」**: `mdait.needsAttention.next` を CodeLens・コンテキストメニュー・キーバインドの3導線で提供。自動ジャンプはしない（UX-P5）。※CodeLens の導線はその後外し、移動先は訳文と原文を並べた対訳表示に変えた（ADR-260912-07）。
