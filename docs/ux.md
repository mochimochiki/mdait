...
>
> 追記 2026-08-02: 非エンジニア層の実機通し（ux-lab）により F-1〜F-9 を追加。**AI ネイティブ運用を前提に置く**という方針転換（UX-P9・ADR-260802-01）に伴い、AI 運用ループに乗らない操作を畳む UX-R5 を起こした。
...
- **UX-P9: AI が使えることを前提に置き、支えない体験を増やさない**（ADR-260802-01）— mdait は AI ネイティブなツールである。**AI 運用ループ（同期 → ✨翻訳 → ✨レビュー → ✨用語集/TM）に乗らない操作は、消すか、他の操作の中に畳む。** 手作業は「このユニットだけ手で訳す」という逃げ道としてのみ支え、手作業だけで完結する運用は支えない。機能を足すときは「これは AI 運用ループのどこに乗るか」を先に答える。答えられないものは足さない。
  - この原則は「AI が無い人を締め出す」ためではなく、**中途半端な非AI経路を増やさない**ためにある。非AI経路を半端に開くと、選択肢だけが増えて到達できない道が生まれる（実例: 用語集は AI 検出でしか作れないのに、空状態のボタンだけが存在していた）。
  - AI の入手手段（Copilot 契約・APIキー取得）は UX の対象外とし、導入ガイドが担保する。
...
| **debug IPC** | 開発/E2E 検証用のファイルベースIPC（`MDAIT_DEBUG_IPC` または `.mdait/debug/.ipc-enabled`）。全 `mdait.*` コマンドを機械実行 | `.claude/skills/mdait-lab/` |
...
| **Decoration**（ユニット右のインライン薄字） | そのユニットの**状態を一言で**（`翻訳完了 : 1.2秒` / `原文が変わりました`） | 操作。長文。折り返すほどの情報。**人が訳文を手で直したこと**（ADR-260905-04） |
...
| **ステータスバー**（右下の常駐） | いま抱えている件数の**要約を1行で**（`$(globe) 3件 翻訳待ち / 2件 要確認`）。クリックで StatusTree へ | 個別の項目。実行結果。操作の説明 |
...
- 状態 → **何も出さない**。触った本人がいちばんよく知っているので知らせる意味が薄く、少し直して保存するたびに「編集済み」と出るのはかえって不安にさせる
- 解説 → 出さない
- 操作 → CodeLens の `✓翻訳済みにする`。`need:translate` のユニットには常に出ているので、これが唯一の案内になる

> かつては Decoration に `編集済み — まだ完了にしていません`、Hover にその解説を出していた（ADR-260801-03）。「訳したのに進捗が動かない」で手が止まるのを防ぐためだったが、判定（`need:translate` かつ `hash≠from`）は人が触ったかどうかを言い当てられず、原文が変わっただけのユニットにも出ていた。判定を直したうえで、**そもそも黙っているのが正しい**と決め直した。
...
最短ループは「保存（自動sync）→ trans → （review）→ tm.commit」の4アクション。UX-P6 に従い、各工程の完了通知が次工程のボタンを持つべきである。現状は sync→trans（翻訳待ちが残るとき「✨今すぐ翻訳」）と sync→review（翻訳待ちが 0 で確認待ちが残るとき「✨AI review」。両方残れば翻訳を先に勧め、ボタンは常に1つ。ADR-260912-02）を実装。trans→review→tm の連鎖誘導は §8 UX-R2。

マーカーの無い既訳は、取り込み（adopt）を経なくてもふつうの sync が `need:review` で受ける（原文の丸写しだけは `need:translate`。ADR-260912-01）。だから確認待ちは定常運用でもふつうに発生し、sync→review の導線はその出口である。
...
| レビューの不採用（`need:review` → `need:translate`） | CodeLens「要翻訳にする」 | —（`mdait_resolve` は採用側だけ。ADR-260912-02 の範囲外） |
...
| B-8 | 裁定後に次の項目へ進む導線がなく、ツリー→ジャンプ→裁定→ツリーの往復が残る（B-3で一覧は得たがキューとして未完成） | 中 | ✅ `mdait.needsAttention.next`（次の要対応へ）を3導線で追加（ADR-260724-01）。※その後 CodeLens の「Next」は外した（ADR-260912-02。マーカー行に並べるのはそのユニットへの操作だけ）。いまの導線は要対応ノードのインライン・パレット・キーバインドで、移動先は訳文と原文を並べた対訳表示（`mdait.openPair`） |
...
| # | 痛点 | 重大度 | 状態 |
|---|---|---|---|
| D-1 | trans 完了後の次アクション誘導がない（need:review が新規発生しても知らされない。tm.commit を思い出すのはユーザー任せ） | 中 | 🔜 UX-R2。sync 完了通知には AI レビューの導線を追加済み（翻訳待ちが 0 で確認待ちが残るとき「✨AI review」。ADR-260912-02）。trans 完了通知は未 |
...
| E-3 | tm-optimize がキャンセル不可（token 未配線） | 低 | ✅ コマンドごと廃止（重みの仕組みをやめた。ADR-260908-01） |
| E-4 | term.add が sourceLang/targetLang を en/ja 決め打ち | 低 | 🔜 UX-R3 |
| E-5 | パッチ適用失敗の Continue/Skip 確認がバッチ翻訳を中断させ、連続失敗で連打 | 低 | ✅ 排他区間では問わず安全側（訳文を据え置き）に倒し、理由つきで区間の外に1回だけまとめて報告。「全文で訳し直す」導線を添える（ADR-260803-01）。実際はモーダルですらなく、押されるまでロックを握ったまま待っていた |
...
### F. 非エンジニア層の実機通しで見つけたもの（2026-08-02・ux-lab）

題材は mdait 未導入の日本語ドキュメントサイト（`docs/` 配下4ファイル、frontmatter・表・コードブロックを含む）。AI 不達の環境で通した。

| # | 痛点 | 重大度 | 状態 |
|---|---|---|---|
| F-1 | **原文と1文字も違わない本文のまま「翻訳済みにする」が通り、警告なく翻訳率に数えられる**（同一テキスト検査が無い） | 重大（バグ） | 🔜 UX-R5 |
| F-2 | `hasUnconfirmedEdit()` が `marker.hash`（訳文テキストのハッシュ）と `revise@X` の X（**旧原文ハッシュ**）を比較しており、別種の値なので実質つねに真。訳文に一度も触れていない revise ユニットにも `編集済み — まだ完了にしていません` が出る | 重大（バグ） | ✅ 解消（ADR-260802-03 で比較対象を正し、ADR-260905-04 で表示ごと廃止） |
| F-3 | 初回 sync が無確認で原文 Markdown を全件書き換える。UI に取り消しは無い | 中 | 🔜 UX-R5（external 既定化で書き換え自体が消える＋確認を追加） |
| F-4 | 画面にハッシュが常時見えている（32行の訳文に5本のマーカー行、入力中も書き換わる）。frontmatter は external でも残る | 中 | 🔜 UX-R5 |
| F-5 | autoSyncOnSave がサイレントで、原文保存により訳が古くなった事実に気づけない（D-2 の再確認。ツリーの数字が減るだけ） | 中 | 🔜 UX-R5 |
| F-6 | 「原文のどこが変わったか」を見る手段が無い。`.mdait/unit-registry` に旧原文が保存済みで AI 不要で出せるのに、どのサーフェスにも出ていない | 中 | 🔜 UX-R5 |
| F-7 | 用語集・TM・検証の入口が `…` オーバーフローのみ。用語集の空状態のボタン（「用語を検出」）はアクティブエディタ必須で実行につながらない | 中 | 🔜 UX-R5（用語操作の統合で解消） |
| F-8 | `✨TM最適化` が非AIなのに ✨ 付き。コストの予測を裏切る | 低 | ✅ 重みの仕組みごと廃止（ADR-260908-01） |
| F-9 | 検証が「15ユニット中14が need 残りでスキップ」でも完了通知は「passed」。スキップ数はレポート本文にしか出ない | 低 | 🔜 UX-R5（AIレビューへ吸収した先で正直に報告する） |
| F-10 | **原文ファイルをリネーム・移動すると訳文が孤立する。** どちらのモードでも破壊はしないが、embedded は状態が残り external は `unit-state` の行が全て消える。ユーザーへの通知は無く、進捗の分母だけが動く | 重大 | 🔜 UX-R5 段階4のゲート（下記） |

**記録のみ（今回は扱わない）**

- 用語照合が大小文字を区別する（`text.includes`）。手書きの用語集では `Audit log` が用語 `audit log` に対して違反になる。AI 検出が variants を供給する前提が UX-P9 で成立するため据え置き
- 運用開始後に既訳サイト（既存の対訳ディレクトリ）を足す動線。adopt をタイトルバーから外すとパレットのみになる。別途検討する価値のある動線として記録する

### G. 操作の実行ライフサイクル（2026-08-03・ボタン操作の通し確認）

| # | 痛点 | 重大度 | 状態 |
|---|---|---|---|
| G-1 | 排他区間の中で「Sync を実行」ボタンを待つため、押すと FileMutex（再入非対応）が自分を待ち**恒久デッドロック**。以後そのファイルの翻訳・同期・保存時自動同期が全停止し、ウィンドウ再読込以外に復帰手段がない | 重大（バグ） | ✅ 区間の外で問う（ADR-260803-01） |
| G-2 | 多重起動を止める仕組みが無く、2回目は待ち行列に並ぶだけ。待った末に「ユニットが見つかりません」で終わり、CodeLens 経由だと成功トーストが重なる | 重大（バグ） | ✅ `OperationRegistry` で断る。通知は結果を見て1回だけ |
| G-3 | 外部マーカー方式で、中断・失敗時に**その実行で翻訳済みのユニットが全消失**（一括保存に到達しない） | 重大（バグ） | ✅ finally で必ず保存（ADR-260803-01） |
| G-4 | 進行中の旗（`isTranslating`）の解除が分岐ごとの手書きで、パッチ据え置き・中断・例外の経路で漏れ、回転アイコンが同期まで止まらない | 中 | ✅ 旗を廃止し表示は台帳に問う |
| G-5 | キャンセルの表し方が層ごとにバラバラで判定できず、正常な中断が赤いエラー＋`Status.Error` になる。ディレクトリ翻訳では `failed` に計上 | 中 | ✅ `OperationCancelledError` 一種に統一 |
| G-6 | パッチ適用失敗の理由を持ち回る型が無く、「なぜ失敗したか」をどのサーフェスにも出せない | 中 | ✅ `PatchApplyResult` で理由を返し通知本文に載せる |
| G-7 | ファイル翻訳で対象0件のとき完全に無言。押しても何も起きなかったようにしか見えない | 中 | ✅ 「翻訳するものはありませんでした」を出す（UX-P7） |
| G-8 | `changeUnitStatus` が `contextValue` を更新しないため、中断後に翻訳済みユニットへ ▶ が残る | 中 | ✅ 終了時に必ずディスクから状態を作り直す |
| G-9 | 非MDファイルは AI 呼び出しが**終わってから**キャンセルを見て結果を捨てる（課金だけして破棄） | 低 | ✅ 呼び出し後は破棄せず保存する |
| G-10 | `updateAndSaveUnit` がマーカー文字列の**最初の出現**を置換するため、同一内容ユニットが同一ファイルに2つあると別ユニットを壊しうる | 低 | 📋 実データでの発生未確認。位置ベースの特定へ寄せるか要検討 |
| G-11 | AI 応答が検証を通らないと**生の応答をそのまま本文に書き込む**（警告は Hover にしか出ない） | 中 | 📋 応答検証の設計判断が必要 |
| G-12 | 翻訳中でもツリー・CodeLens の ▶ は押せたままで、押すと「いま翻訳中です」と断られる。sync は `mdaitSyncProcessing` でボタン自体を差し替えており非対称（UX-P7「実行できない操作はサーフェスに出さない」） | 中 | 📋 `when` 句が `viewItem == X` の完全一致で書かれており、busy を表すには 8 箇所の条件式と contextValue の設計変更が要る。断る動線は正直に働くので別途 |
| G-13 | 用語集更新は台帳を表示（スピナー）にしか使っておらず、多重起動を断らない | 低 | 📋 断る対象を翻訳に限る判断。用語集の多重起動は元から拒否していない |
| G-14 | ユニット翻訳の通知が対象をハッシュ先頭8桁で呼ぶ（`ユニット 1a2b3c4d`）。利用者にとって意味を持たない識別子 | 低 | 📋 見出しを持ち回れば直せる |
| G-15 | ファイル翻訳中は配下の全ユニット行が回転する（以前は処理中の1件だけ）。「いまどこを訳しているか」の解像度が下がった | 低 | 📋 台帳の粒度を上げるか、進捗表示側で補うかの判断 |
...
3. term.add の言語 QuickPick 化（transPairs から候補生成）。
...
4. **「次の要対応へ」**: `mdait.needsAttention.next` を CodeLens・コンテキストメニュー・キーバインドの3導線で提供。自動ジャンプはしない（UX-P5）。※CodeLens の導線はその後外し、移動先は訳文と原文を並べた対訳表示に変えた（ADR-260912-02）。
...
### UX-R5: AI ネイティブ運用への収束（F-1〜F-9）— ADR-260802-01〜04

**目的**: UX-P9 を実装レベルで通す。AI 運用ループに乗らない操作を畳み、画面からハッシュを消し、原文が変わったことに気づける状態にする。**AI が使える前提**で設計し、AI の入手手段は導入ガイドが担保する。

**4段階で出す**（1〜3は表示と露出の変更、4は保存形式の変更なので独立して出す）:

1. **消す・畳む**（F-7〜F-9）
   - 非AI検証 `mdait.validate` の人間導線を廃止し、✨AIレビューの**前処理**として吸収する（決定的な違反を先に洗ってから AI に投げる＝AI コストが下がる）。エージェント向け `mdait_validate` はゴール判定に使うため据え置き
   - `✨用語検出` と `✨用語をこの言語へ展開` を **「✨用語集を更新」1つに統合**する（検出→展開を続けて実行）。「検出」「展開」は実装都合の語彙であり、用語集が2段階で作られることをユーザーが知る必要はない
   - `✨TM最適化` を廃止する（重みは手元の原稿からしか決まらない派生値で、共有資産に載せると合流のたびに全件がぶつかる。ADR-260908-01）
   - `マーカーを外部化 / 埋め込み` をパレット非表示にし、設定変更時は sync の自己修復に委ねる
   - `✨既存翻訳の取り込み`（adopt）は**未同期のときだけ**タイトルバーに出す（E-7 の誤操作誘発を解消）
   - `対象言語を選択` は transPair が2つ以上のときだけ出す
   - `sync.level 0`（完全手動マーカー配置）は設定エディタで非表示にする（値は互換のため残す）
2. **バグ修正**（F-1・F-2）— 同一テキストのままの確定を止める。`hasUnconfirmedEdit()` の比較対象を正す
3. **気づきと差分**（F-5・F-6）— ステータスバーに needs 集計を常駐。訳文側ユニットに Decoration `原文が変わりました`、Hover に旧原文との差分。**原文側には出さない**（原文を書いている間は訳文を意識させない）
4. **ハッシュの消去**（F-3・F-4）— frontmatter マーカーも `unit-state` へ退避し、新規セットアップの既定を `markers.mode: external` に。原文は初回同期でも書き換わらなくなる。**未着手（下記のゲートで保留）**

#### 段階4のゲート: リネーム・移動で状態が失われる（実機確認済み・2026-08-02）

原文ファイルを VS Code のエクスプローラでリネームし、sync した結果:

| モード | 訳文ファイル | 訳文の状態（マーカー/unit-state） | 進捗表示 |
|---|---|---|---|
| embedded（現既定） | 残る（消えない） | **残る**（マーカーが本文と一緒に動くため） | 旧ペアが孤立し分母が増える（15→19） |
| external | 残る（消えない） | **全行が消える**（`unit-state` はパスをキーにするため） | 訳した1件が 0 に戻る（1/15→0/15） |

どちらも**破壊はしない**（訳文の本文は残る）が、external は状態を失う点で embedded より弱い。
**この差を埋めずに既定を external にしてはならない。** 段階4の前提条件は次のとおり:

- リネーム・移動を検知して `unit-state` の行を追随させる（内容ハッシュでの再リンク、または git の rename 検出）
- 追随できなかった場合は**削除ではなく要対応**に倒し、「この訳文は原文と結びついていません」と可視化する
- 孤立したペアが生まれたことを、sync 完了通知かツリーで伝える（現状はどちらのモードでも無言）

なお、この「リネームで訳が孤立する」問題は external 固有ではなく**現行の embedded 既定にも存在する**（分母だけが増え、誰も気づけない）。段階4の前提条件は、そのまま現行の欠陥の修復でもある。

**初回体験は「同期 → ✨翻訳」の2段を維持する**。2回目以降も必要な手順であり、最初から同じ手順を辿らせるほうが学習が1回で済む。1操作への統合はしない。

**完了ゲート**:

- ツリー行・タイトルバー・パレットに出る操作が、すべて AI 運用ループ（同期 / ✨翻訳 / ✨レビュー / ✨用語集 / ✨TM / 判断）のいずれかに分類できること（例外なし）
- 原文と同一テキストのユニットを「翻訳済み」にできないこと（単体テスト）
- ~~訳文に触れていない `need:revise` ユニットに「編集済み」が出ないこと（単体テスト）~~ → 「編集済み」の表示自体を廃止した（ADR-260905-04）
- 段階4は、**ファイルのリネーム・移動で訳文が消えないこと**を実機で確認してから出す。状態が迷子になった場合は削除ではなく要対応へ倒す
