...
| 操作 | 破棄＝ごみ箱へ移動（modal）。`unit-mutation.ts` の `discardTargetFile` 経由（ADR-260726-01） |
...
- [x] 敵対的レビュー（不変条件。正しさ・エッジケースの2体は API 上限で中断）
...
**評価:** ⭐3 → ⭐4（不変条件レビュー1周ぶん反映済み）

- [x] 🔴 非MD訳文が全体再構築の経路で孤立の印を落とす（`collectAllFromDirectory` が拡張子で分岐し `collectFileStatus` を迂回）
  - 分岐を廃止して `collectFileStatus` へ一本化。エラー時のフォールバック項目にも印を付ける
- [x] 🟠 ピボット構成（`ja→en, en→fr`）で現役の原文を「原文なし」と印し、破棄まで提示していた
  - probe の `deriveSourcePath` が、そのパス自身がどれかのペアの原文なら null を返すようにした（単体テスト追加）
- [x] 🟠 `mdait_getStatus` に孤立が出ていない（ADR-260806-01 の決定が未実装だった）
  - `StatusData.orphanTargets` と summary・nextActions に追加。破棄の手段は渡さない
- [x] 🟠 `discardTargetFile` が未保存バッファを反映していない（消した直後の保存で「行の無い訳文」が復活する）
  - `flushDirtyDocument` を追加。あわせて孤立チェックを関数の内側にも置いた（`config` は死に引数だった）
- [x] 🟡 破棄したパスが孤立の通知記憶に残り続ける → `forgetOrphanPath` を追加
- [x] 🟡 行ごとに `existsSync` が飛ぶ（1ファイル100ユニットで200回）→ probe の生存期間だけ結果を覚える
- [x] 🟡 `totalTargetEmptied` が `SyncResult` とログに無い／`ADR-260803-06` のコメントが腐っていた → 両方修正
- [x] 🟡 サイズ超過など別の `tooltip` があると孤立の解説が消える → 孤立を先に見て連結する
- [x] 🟡 コマンドの孤立チェックが try の外で、ワークスペース未設定だと生の例外で落ちる → 捕捉して案内に変える
- [x] 🟡 `docs/design/ui.md` の contextValue 一覧に `mdaitFileTargetOrphan` が無い → 追加
- [ ] 🟡 1回の sync でトーストが最大5本重なりうる（今回2本増やした）。集約は別途判断
- [ ] 🟡 読み上げラベルの副題漏れは**誤指摘**だった。tooltip 側が状態を語っており、両方入れると二重に読む（テストが検出）
