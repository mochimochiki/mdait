# translate-selectionコマンド設計

> **上位設計**: [commands.md](commands.md)

## 役割

エディタの選択範囲をmdait管理外で一時的に翻訳する軽量機能です。マーカーレス・履歴管理なしで、mdaitのステータスには影響しません。

**設計意図**: プロジェクトメンバー間の軽いコミュニケーション（Slackへの投稿、コードコメント等）を翻訳したい場合など、正式な文書翻訳ではない用途に対応します。mdaitの本来の管理対象（マーカー付きMarkdown）とは分離することで、ステータス管理を汚染しません。
