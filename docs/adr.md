...
### 決定
1. **通知の一本化**: `StatusItemTree` の変更系メソッドは個別発火をやめ「変更あり」1本のシグナルに集約する。`StatusManager` がデバウンス（既定80ms・trailing edge 保証）で束ね、`fire(undefined)`（ツリー全体再描画）のみを発行する。部分通知と `notifyRootChanged()` は**廃止**する。「どのノードを描き直すべきか」という判断こそがバグの発生源であり、その判断を設計から削除する。
...
4. **要対応の意味論**: 対象は `need:review` / `need:verify-deletion` のみを維持し、**選択中の transPair に属するファイルのみ**へ絞る（集計関数には対象ディレクトリ集合を引数で渡し、core に `SelectionState` 依存を持ち込まない）。AIレビューの flagged 統合は受理台帳を前提とする別課題として据え置く（ux.md B-4）。
5. **キューの完成**: 要対応項目に `description`（ファイル名・種類）と安定ソート（相対パス→startLine）を与え、`mdait.needsAttention.next`（次の要対応へ）を CodeLens・コンテキストメニュー・キーバインドの3導線で提供する。自動ジャンプはしない。

### 理由
呼び忘れた通知を足すだけの修正は、同じ穴を再発させる（要対応ノードは ADR-260712-03 で追加されたばかりなのに既にこの穴が空いた）。一方で状態管理そのものの全面再設計は、全コマンドが `StatusManager` を直接呼ぶ現状では変更量が過大でプレリリースの速度を損なう。通知設計だけを作り直す中間案が、変更を3ファイルに閉じ込めつつ再発を構造的に防げる。部分描画の最適化を捨てるのは、その最適化に必要な「影響範囲の算出」が新たなバグ置き場になるためで、実際に今回その算出誤り（要対応ノードは影響外という暗黙の判断）が不具合を生んでいる。索引の「毎回張り直し」を選んだのも同じ理由で、「消す処理を足す」方式は消し忘れケースを人間が列挙し続ける必要がある。
...
- 挙動変化: これまでツリーに残り続けていた削除済みファイルが消える。要対応から選択外の言語ペアの項目が消える。いずれも CHANGELOG に記載する。
