...
- **人間とエージェントで要対応の件数が異なりうる**: 本 ADR で人間側（StatusTree）を選択中の transPair に絞ったが、LM Tools（`mdait_getStatus` 等）は従来どおりワークスペース全体を集計する。エージェントはサイト全体の完成を目標にするため全体観測が適切だが、UX-P1（サーフェス対称性）に対する意図的な非対称であり、混乱を招くなら統一を検討する（ux.md B-9 として台帳に記録）。
