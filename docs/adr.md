...
## ADR-260724-01: ステータスツリーの更新を全体再描画＋デバウンスに一本化し、ユニット実体を children に一元化する

### 背景
「翻訳後に要対応が1件しか出ていないのに、訳文ツリーを展開すると項目が増える」という報告を調査したところ、構造的欠陥が判明した。「要対応（Needs Attention）」ノードの件数と中身は**ルートの子リストを組み立てるときにだけ**計算される（`StatusTreeProvider.buildNeedsAttentionItem` / `getNeedsAttentionChildren`）が、ツリー更新通知はほぼすべてが「このディレクトリ配下だけ」の部分通知であり（`StatusItemTree.updateDirectoryAggregatesUpward` が再帰の最上位でのみ発火）、ルート全体の通知はめったに出ない。そして**要対応を増やす処理（trans の `need:review` 付与・sync の孤立ユニット一次受け／`verify-deletion`・AIレビューの escalate）はいずれもルート通知を出していない**。`notifyRootChanged()` を呼んでいるのは要対応を減らす側の3コマンドのみだった。手動 sync だけは後続の `buildStatusItemTree()` で全再構築されるため直り、自動 sync と翻訳では直らない ——「押すと直る／放っておくとズレる」の正体である。

同じ原因系に、(a) 1ファイル更新でルートディレクトリ配下全体を無効化する過剰な部分通知、(b) 消えたユニット／ファイルを索引から取り除く手段がなくゴーストが残る、(c) `refreshFileStatus` 後に同一ユニットの実体が索引側と `children` 側で2つに割れる、(d) 要対応だけが選択中の transPair で絞られない、という欠陥が同居していた。

### 決定
1. **通知の一本化**: `StatusItemTree` の変更系メソッドは個別発火をやめ「変更あり」1本のシグナル（ペイロードなし）に集約する。`StatusManager` がデバウンス（既定80ms）で束ね、ツリー全体再描画の通知のみを発行する。部分通知と `notifyRootChanged()` は**廃止**する。「どのノードを描き直すべきか」という判断こそがバグの発生源であり、その判断を設計から削除する。デバウンスには上限待ち時間（既定300ms）を設け、ディレクトリ一括 sync のように更新が途切れず続く処理の最中もツリーが凍らないようにする。
2. **前提の明文化**: 全体再描画が安価であること（`getChildren`/`getTreeItem` はインメモリ参照のみ・可視ノード限定・ディスクI/Oを伴うのは `buildStatusItemTree()` だけ）と、展開状態の維持が `treeItem.id` の安定性に依存することを `docs/design/ui.md` に明記し、id 採番の変更には回帰確認を課す。
3. **実体の一元化**: `FileStatusItem.children` を唯一の本体とし、`unitItemMapWithPath` は索引に降格してファイル単位で毎回張り直す。`removeFile(filePath)` を追加し、`refreshFileStatus` の不存在検知から自己修復的に呼ぶ。
4. **要対応の意味論**: 対象は `need:review` / `need:verify-deletion` のみを維持し、**選択中の transPair に属するファイルのみ**へ絞る。集計関数（`StatusItemTree.getNeedsAttentionUnits`）には対象ディレクトリ集合を引数で渡し、その算出（`getSelectedScopeDirs`）は commands 層に置く。core（純粋な翻訳ロジック）が UI セッションの選択状態を知らない状態を保つためである。AIレビューの flagged 統合は受理台帳を前提とする別課題として据え置く（ux.md B-4）。
5. **キューの完成**: 要対応項目に `description`（ファイル名・種類）と安定ソート（ファイルパス→startLine。ロケール非依存の素の比較）を与え、`mdait.needsAttention.next`（次の要対応へ）を CodeLens・コンテキストメニュー・キーバインドの3導線で提供する。自動ジャンプはしない。CodeLens からはクリック行を引数で渡す（CodeLens のクリックはカーソルを動かさないため、渡さないとカーソル位置が起点になり前へ戻る）。

### 理由
呼び忘れた通知を足すだけの修正は、同じ穴を再発させる（要対応ノードは ADR-260712-03 で追加されたばかりなのに既にこの穴が空いた）。一方で状態管理そのものの全面再設計は、全コマンドが `StatusManager` を直接呼ぶ現状では変更量が過大でプレリリースの速度を損なう。通知設計だけを作り直す中間案が、変更を状態管理まわりに閉じ込めつつ再発を構造的に防げる。部分描画の最適化を捨てるのは、その最適化に必要な「影響範囲の算出」が新たなバグ置き場になるためで、実際に今回その算出誤り（要対応ノードは影響外という暗黙の判断）が不具合を生んでいる。索引の「毎回張り直し」を選んだのも同じ理由で、「消す処理を足す」方式は消し忘れケースを人間が列挙し続ける必要がある。

### 備考
- 却下案: `need:translate` / `need:revise` も要対応に含める案は、AI に投げれば片付く機械作業の中に人間待ちの項目が埋もれるため却下（キューの価値は「AI に投げても解決しないものだけが並ぶ」ことにある）。種類別のグループ分けは、種類が2つしかない割に階層を深め、空グループがデッドエンド（UX-P7）を生むため却下。
- 挙動変化: コマンドが触れたファイルが消えていればツリーからも消える（ファイル監視は無いため、mdait 外での削除は次に触れるまで残る）。要対応から選択外の言語ペアの項目が消える。いずれも CHANGELOG に記載する。
- **人間とエージェントで集計範囲を揃えた**: 当初は人間側（StatusTree）のみを選択中の transPair に絞ったが、LM Tools（`mdait_getStatus` / `mdait_sync` / `mdait_aiReview` / `mdait_adopt`）の集計も同じ範囲に統一した。sync・trans は元から選択中のペアだけを処理するため、集計だけが全ペアを数えると「エージェントには見えるが誰も処理しない件数」を報告することになり、エージェント surface の内部でも矛盾していた。範囲の算出点は `getSelectedScopeFiles` 1か所に集約し、`mdait_getStatus` は集計範囲を summary のラベル（`workspace (targets: ja)`）で明示する。**パスを明示指定した場合は絞り込まない**（明示指定はユーザー/エージェントの意思表示として尊重する）。
- 実装チケット: `.tasks/do/260724-01_要対応キューと状態同期の再設計.md`。課題台帳は ux.md B-5〜B-8 に登録。
...
### 背景
エージェント・オーケストレーションの完成条件（needs 全0）に `need:review` / `need:verify-deletion` の解決が含まれるのに、その解決手段は人間の CodeLens（Mark as …）にしか開かれておらず、エージェントはマーカー文字列の直接編集（プレイブックで唯一許容されていた手書き操作）を強いられていた。マーカー手編集は hash/from 破損のリスクを伴い、「全操作の機械到達性」（[ux.md](ux.md) §5）を欠く最大の穴だった。

### 決定
LM Tool **`mdait_resolve`** を新設し、指定ファイル（＋任意で unitHashes / need 種別フィルタ）の need フラグ除去を CodeLens と同一のコマンド層ロジックで実行できるようにする。hash/from は変更しない。translate/revise の解決は明示指定時のみ許可（既定は review/verify-deletion）。マーカー書換のため確認 UI を設け（AI 不使用）、冪等（2回目は resolved 0 件）。プレイブックの「マーカー手書き編集」の記述は本ツール使用に置換する。

### 理由
人間とエージェントのサーフェス対称性（同じ判断を同じ意味論で実行できる）を確立し、壊れやすいマーカー手編集をエージェントの正規手順から排除するため。判断の委任境界は変えない（確認 UI 維持・自動連鎖なし）。

### 備考
- 同時に `mdait_getStatus` の `detail:true` にユニット単位の need 一覧（hash/title/need、上限付き）を追加し、「どのユニットを resolve するか」の観測→行動が閉じるようにした（agent-orchestration G-2 の残り）。
- isolate 宣言（need 付与方向）は本ツールの範囲外。UX-R1（[ux.md](ux.md) §8）で判断サーフェスと合わせて設計する。

## ADR-260712-01: UX基準ドキュメント docs/ux.md を新設し UX 原則を明文化する

### 背景
UX に関する記述が ui.md（部品カタログ）・architecture.md P6（原則1項）・各 command_*.md（コマンド別導線）・個別 ADR に4分割で散在し、「ジャーニー全体」「人間とエージェントのサーフェス対称性」「課題の全体像」を俯瞰する場所がなかった。全数調査（2026-07-12）で、ディレクトリ操作の恒常的失敗（dirPath/directoryPath 取り違え）・パレット露出コマンドの破綻・英語 l10n の生キー表示・needフラグ可視化の欠落など、横断視点の不在に起因する劣化が複数発見された。

### 決定
1. **[docs/ux.md](ux.md) を UX の基準ドキュメントとして新設**する。UX 原則（UX-P1〜P7: 状態の全サーフェス観測可能性・判断サーフェスの原則・冪等復帰・AI明示起動・VS Code標準準拠・工程間の手渡し・デッドエンド禁止）、人間/エージェント双方のジャーニー、状態可視化マトリクス、課題台帳、改善ロードマップ（UX-R1判断サーフェス / UX-R2工程間誘導 / UX-R3操作一貫性）を集約する。UX に関わる変更は本ドキュメントとの整合を確認する。
2. 調査で特定したデッドエンド・破綻を同日修正: ツリーのディレクトリ TM commit / AI レビューのバグ修正、アイテム引数必須コマンドのパレット非表示化、▶（Translate Unit）を trans が処理するユニットに限定、need:revise/verify-deletion/isolate のツリー専用表現追加、英語 AI 確認ダイアログの実文化。

### 理由
プレリリース段階の開発重心がオンボーディング・命名・設定 UI など UX に移っており、横断基準がないまま個別最適を続けると同種の劣化が再発するため。原則の明文化により「実行できない操作はサーフェスに出さない」等の判断を機械的に再適用できる。

### 備考
- ロードマップの各リリース（UX-R1〜R3）は agent-orchestration.md のマイルストーン形式で個別チケット化して実装する。
- 課題台帳の 📋（記録のみ）は発生頻度・ユーザーフィードバックを観測してから昇格を判断する。
