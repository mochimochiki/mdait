...
- **実装位置**: `marker-sync.ts` の新規ターゲット分岐に「本文が空でなければreviewに倒す」判定を追加する。externalモード再構築（当時の `sync-command.ts` の `isExternalRebuild`）と非MD（`plain-file-handler.ts`）に既にある同種ロジックを共通化して流用する。→ 現在は `needForFirstLink` に一本化済み（上記の更新）。
...
>
> **検証の進み（2026-09-23）**: Copilot Chat が要らないものは mdait Lab で確かめた（`lab run mdait_*` で LM Tool を
> Copilot Chat と同じ順に呼べるようにした）。
> - **並列翻訳（M6）**: 100ファイル・300ユニットのディレクトリ翻訳を、応答に 500ms かかる偽の AI で測った。
>   並列1 で 152秒、並列3 で 52秒（2.9倍）、並列8 で 20秒（7.6倍）。どれも翻訳待ち 0 で終わった
> - **P11（新規翻訳の通し）**: 最後の sync は added / modified / deleted / revisionsNeeded / adopted がすべて 0、
>   term と tm の2回目は新規 0。偽の AI は原文をそのまま返すので、品質チェックで `need:review` が残るのは道具の限界
> - **P12（取り込みの通し）**: 既訳の本文は1文字も変わらない（付いたのは frontmatter のマーカーだけ）。
>   `40_structure_mismatch.md` の誤ペア（原文だけの章と訳文だけの章が位置で組まれる）は下の「着手前チェック」の懸念どおり出る
> - **受け入れテスト（M6）**: 実装を読まない別のエージェントに手順書（guide-developer.md）だけを渡し、LM Tool 越しに
>   S2 をやらせた。基本ループ（観測 → 1つ実行 → 観測）と「知識構築が先」は手順書だけで辿れた。完成状態には届かなかった —
>   偽の AI の訳が `need:review` で残るのは道具の限界だが、ほかに4つの穴が出た。(1) `mdait_resolve` が知らない `action` を
>   resolve として扱い、確かめる前の訳を承認できた → 弾くよう直した。(2) Git の管理下でないときの扱い、
>   (3) `need:review` のままでは `declare-isolate` が効かない順序、(4) AI レビューが失敗すると `escalations` が空になり、
>   位置ずれの誤ペアに機械では気づけないこと → 手順書に書き足した。(4) の誤ペアを人が直す手段は、まだ手作業しか無い
> - **残るのは Copilot Chat の実機確認だけ**（各ツールの呼び出し・JSON の扱い・「ひとこと依頼」での通し）
...
4. mdait-lab（`.claude/skills/mdait-lab/`）でS1・S2のE2Eシナリオを構築し、「ひとこと依頼→完成状態」を通しで検証（パターン集の P11 / P12）
