...
- レポート: 実ファイル `.mdait/local/reports/ai-review.md`（Markdown 表）。mismatch を先頭にソートし、**自動承認したユニットも必ず列挙**する（TM 登録可能状態への昇格を可視化）。見出しは `buildReviewReport` へのラベル注入で表示言語化する（件数の語彙行・表ヘッダ・verdict/action 語彙は共通語彙として英語固定。ADR-260719-01）。ユニット列は該当箇所への行リンクになる（`linkBaseDir` にレポートの置き場所を渡す。取り込みウィザードと同じ）
