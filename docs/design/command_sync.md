...
- **GC**: UnitRegistry合計5MB超過時のみ実行。未参照スナップショットを削除。ただし**設定の全ディレクトリに見に行けて（0件でも可）、取り消されず、`unit-state` と `unit-registry` を丸ごと読めた回だけ**（ADR-260908-06。詳細は [core.md](core.md) の GC処理）
