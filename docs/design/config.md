...
| `trans.maxUnitsPerRun` | `300` | 1ファイルの処理で扱うユニット数の上限（全般コストガード）。**ファイル単位で適用**され、trans・aiReview・AIアライン（adopt）が共通で参照する（ディレクトリ実行ではファイル数ぶん積み上がる）。超過時の挙動は経路ごとに異なる: trans / review は超過ユニットの need フラグ（`need:translate` / `need:review`）を保持し次回実行で処理、align は該当ファイルの AI align をスキップして位置ベース対応付けを維持。`0` で上限なし |
...
| 設定 | デフォルト | 動作 |
|------|-----------|------|
| `aiReview.autoApprove` | `true` | 高確信 match の `need:review` を自動解除。`false` でレポートのみ（マーカー無変更）のセーフモード |
| `aiReview.batchSize` | `3` | 1回のLLMコールで検証するペア数（1..10 クランプ）。`1` で従来の単ペアプロンプト（`aiReview.verifyPairing`）、`2` 以上でバッチプロンプト（`aiReview.verifyPairingBatch`） |

旧キー `aiSync.review.*` は廃止（フォールバック無し・ADR-260711-06）。
...
`mdait.json` を書き換えるコードは必ず `infra/config/config-json-editor.ts` を通します（設定画面・マーカー保管方式の一括変換）。

**書き換えるのは触ったキーの値だけで、他のバイト列は1バイトも動かしません**（ADR-260908-06）。位置を測る係は `infra/config/json-text-locator.ts` に分けてあります。

かつては `JSON.parse` → 書き換え → `JSON.stringify` と全体を組み直していました。組み直しは書き方の癖をすべて既定へ倒すので、人が手で書いたインライン配列が展開され、**1項目の変更で15行の設定が57行に膨らみ、git の差分が 52 追加 / 10 削除**になっていました（実測）。設定ファイルは git 管理下にあるので、その差分はそのまま他人の変更とぶつかります。いまは同じ手順で 1 追加 / 1 削除です。

- 既存のキー: 元が1行で書かれていれば1行のまま差し替える
- 追加: 親オブジェクトの末尾へ1メンバーぶんだけ足す
- 削除: その1メンバーと区切りのカンマだけを落とし、空になった親は畳む

差し替えたあとは必ず読み直し、**組み直しで得られるはずの値と一致しなければ組み直しへ落とします**。位置合わせを外したときに壊れた JSON を書くくらいなら、行が動いても正しい JSON を書くほうがましだからです。インデント文字・改行コード・末尾改行の引き継ぎ（ADR-260907-05）は組み直しの側に残してあるので、落ちた回もそこは守られます。
