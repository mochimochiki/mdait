...
**走らせるのは、その回の走査が全部に届いたときだけです**（ADR-260908-06）。控えはハッシュだけを鍵にした表で、どのファイルの控えかを持っていません。見に行かなかった場所から参照されている控えも「使われていない」と読まれて消え、削除は競合を出さずに全員へ伝播します。見送るのは次の4つです。

- 設定のディレクトリのどれかに**見に行けなかった**
- sync が途中で取り消された
- `unit-state` を1行でも取りこぼした（合流の途中）
- `unit-registry` を1行でも取りこぼした（同上）

「見に行けた」は**ファイルの列挙が返ってきたこと**で、0件でも届いています。孤立掃除の `scannedDirs`（1件以上見つけたもの）とは別の集合を渡します — 混ぜると、まだ訳文の無い言語を1つ足しただけで掃除が永久に走らなくなります。対象ペアを絞って走らせた回は「届いていない」に入るので走りません（絞った先の控えを消さないための意図した振る舞いで、台帳が育つだけです）。台帳の傷は「そのとき読んだファイル」の話なので、次に掃除を試すときは読み直して測り直します。
