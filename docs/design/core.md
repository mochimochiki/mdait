...
**マーカーと見出しのあいだの空行は読み飛ばします**（ADR-260908-07）。Prettier・markdownlint・エディタの整形はマーカーの直後へ空行を入れるため、空行を挟むと統合しない読み方では、原稿を整形しただけで**章の数だけ中身の空の幽霊ユニットが生え、見出しは自分のマーカーを失います**（実測: 2章の原稿が4ユニットになり、どちらのハッシュも消えた）。読み飛ばすのは空行だけなので、本文が挟まればマーカー単独の境界のままです。書き出しでは空行を落として正規形へ戻すので、整形された原稿は次の sync で直ります。
...
**走らせるのは、その回の走査が全部に届いたときだけです**（ADR-260908-05）。控えはハッシュだけを鍵にした表で、どのファイルの控えかを持っていません。見に行かなかった場所から参照されている控えも「使われていない」と読まれて消え、削除は競合を出さずに全員へ伝播します。見送るのは次の4つです。
