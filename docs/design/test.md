...
- `lab merge`: 合流の実験場。`.mdait/unit-state` と `.mdait/unit-registry` を枝ごとに違う編集で書き出し、git の3方向マージ・GNU diff3（SVN 相当）・union の3通りで合流させて、**製品と同じローダー**で読み直す。競合の数だけでなく「競合ゼロなのに状態が消えた」行を数えるのが本体（`docs/design/unit-state.md` §26）。VS Code も AI も使わないのでセッションを立てずに走る
