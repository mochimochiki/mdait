...
|---|---|
| `empty-patch` | 答えが空 |
| `unrecognized-format` | 編集ブロックが1つも無い |
| `no-changes` | 当てても前回訳文と同じ |
| `unterminated-block` | `END` で閉じていない（残り全部を本文として飲み込むと訳文の末尾が入れ替わる） |
| `bad-range` | 指した行が訳文に無い（範囲外・逆順） |
| `overlapping-ops` | 2つの指示が同じ行を取り合っている（当てる順序で結果が変わる） |
| `line-number-residue` | 渡した行番号を本文にそのまま書き戻している |

最後の1つは**当てはめ器から見れば正しいパッチ**で、放っておくと `12<TAB>本文` という行が訳文として
保存されます（ADR-260908-05）。判定は位置まで見て行うので、1列目が数字のタブ区切りデータ
