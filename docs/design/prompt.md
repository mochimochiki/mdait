...
#### 概要
原文差分がある改訂翻訳時に、**前回訳文への編集指示のみ**を返却させるプロンプトです。全文再生成を避け、差分外の文は維持します。

#### 設計意図
これが[design.md](../design.md) P4「LLMをdiff-aware reviseの主戦力とする」の中核です：
- 原文の変更差分（unified diff）と**行番号を振った**前回訳文をLLMに提示
- 訳文への編集指示のみを生成させる
- 変更箇所以外は既存訳文を維持し、人間の修正を尊重

**前回訳文を1行も写させない**のが要点です（ADR-260903-01）。旧来は `=`/`-`/`+` のプレフィックスで
文脈行を逐語で写させていましたが、目印が Markdown とぶつかる形が3つあり
（空行に目印を付けられない・箇条書きの `-` と削除の `-` が区別できない・表の行が切れる）、
ローカル2機種の実測で成立4割・実際に改訂が入ったのは19%でした。行番号で指す形式は
その要求そのものを持たないため、これらの衝突が構造的に起きません（成立97%・改訂も97%）。
...
- `{{surroundingText}}`: 周辺テキスト（オプショナル）
- `{{terms}}`: 用語集（オプショナル）
- `{{numberedPreviousTranslation}}`: **行番号とタブを付けた**前回訳文（**必須**）
- `{{sourceDiff}}`: 原文の変更差分（unified diff形式、**必須**）

#### Output

**JSON ではなく素のテキスト**を返させます。封筒はフェンス包み（実測で 12/12）とエスケープ負荷を招き、
後者は逐語コピーを実際に壊していました。
...
- 行番号は1始まりで、渡した前回訳文の見た目どおりを指す
- ブロックは必ず `END` で閉じる。順序は自由だが、**2つのブロックが同じ行を指してはいけない**
- ブロックの中身は**その場所の最終形**。`-`/`+` のような差分の目印を付けず、古い文言を残さない

**用語候補（termSuggestions）は改訂では集めません。** 測った指示文に含めていないためです
（改訂は数行の編集で、その章は初回翻訳時に既に用語を出しています。用語集の構築には
`term.detect` / `term.expand` があります）。

#### 当てない場合（`applyRevisionPatch` が返す理由）

当てはめは [`src/core/diff/diff-generator.ts`](../../src/core/diff/diff-generator.ts) の
`applyRevisionPatch(prev, patch, format)` が唯一の入口です。**形式は引数で決まり、中身からは
推測しません。** 次のいずれかなら当てず、理由を付けて戻します。

| 理由 | 形式 | 何が起きたか |
|---|---|---|
| `empty-patch` | 両方 | 答えが空 |
| `unrecognized-format` | 両方 | その形式として読めない（行番号方式なら編集ブロックが1つも無い） |
| `no-changes` | 両方 | 当てても前回訳文と同じ |
| `unterminated-block` | linenum | `END` で閉じていない（残り全部を本文として飲み込むと訳文の末尾が入れ替わる） |
| `bad-range` | linenum | 指した行が訳文に無い（範囲外・逆順） |
| `overlapping-ops` | linenum | 2つの指示が同じ行を取り合っている（当てる順序で結果が変わる） |
| `line-number-residue` | linenum | 渡した行番号を本文にそのまま書き戻している |
| `anchor-not-found` | prefixed | 目印にする周辺行が訳文側に見つからない（訳文が手で編集された等） |

`anchor-not-found` は**カスタムプロンプトを置いているときだけ**出ます（下記「後方互換」）。
行番号方式は前回訳文の文字列を照合しないので、この失敗の形そのものがありません。
なお `no-source-diff` は当てはめの手前 — 旧原文が無くて差分を作れなかった段 — の理由で、
`applyRevisionPatch` は返しません。

`line-number-residue` は**当てはめ器から見れば正しいパッチ**で、放っておくと
`12<TAB>本文` という行が訳文として保存されます（ADR-260908-05）。判定は位置まで見て行うので、1列目が数字のタブ区切りデータ
（`.tsv` も訳す対象です）を巻き込みません。この形は AI の答えを検証する段でも見ていて、
そこで気づけたときは**同じ形式のままもう一度聞き直します**。

パッチ適用に失敗した場合は**訳文を据え置き**、理由を報告します（全文再翻訳へは自動で落ちません）。
**より簡単な形式で聞き直す段は作っていません** — 実測で行番号方式の成立率は 97〜100% あり、
残る取りこぼしは同じ形式のやり直しで足ります。形式を増やすと、どちらの形式で読むかを推測する
必要が生まれ、「読めてしまうが正しくない」を招きます（ADR-260908-05）。

非MD ファイル（`trans.revisePatchPlain`）だけは、当てはめに失敗すると全文翻訳へ倒れます。
ユニット分割が無く、据え置くと訳文が古いまま残るためです。

#### カスタムプロンプトを置いている場合（後方互換）

`mdait.json` の `prompts` で `trans.revisePatch` / `trans.revisePatchPlain` を上書きしていると、
**その指示文は旧来の `=`/`-`/`+` 形式に向けて書かれている**はずなので、mdait は返答を旧形式として読みます。
判定はファイルの有無だけで行い、中身からは推測しません（推測させると、一方の形式のつもりで
書かれた答えをもう一方として「読めてしまい」、当たったように見えて本文が壊れます）。

行番号方式を自分の指示文で使いたい場合は、上書きを外して組み込みの指示文を使ってください。
