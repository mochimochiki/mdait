...
- [x] 表を見て、二次へ進める候補を決める（`current` と `linenum` の一騎打ちに絞った）
...
- [x] 「改訂が入っていないのに成立する」壊れ方を `health` で捕まえるようにする（行数の判定）
- [x] 結果を ADR に記録する（ADR-260902-01）

**Gates**
- [x] 勝った形式が、12ケースすべてで `health` まで通る（`linenum` 35/36・落ちた1件は別要因）
- [x] 落ちた場合の理由が段ごとに説明できる（下の内訳のとおり）
...
#### 実機での測定（2026-09-02・Qwen3.6-35B-A3B・n=36）

**ここで崖が出た。** 使ったのは `bartowski/Qwen_Qwen3.6-35B-A3B-GGUF:Q4_K_M`（35B MoE・活性3B）を
llama.cpp b9935 / llama-swap の OpenAI 互換の口で立てたもの。1往復あたり25〜31秒。
`--repeat 3`（12ケース × 2候補 × 3回 = 72往復）で振れを潰した数字を採る。

| 候補 | 成立 | intent | フェンス包み | 文字数(中央) |
|---|---|---|---|---|
| `current` | **15/36（42%）** | **7/36（19%）** | 6/36 | 698 |
| `linenum` | **35/36（97%）** | **35/36（97%）** | 0/36 | 218 |

`current` の落ちた内訳: `anchor-not-found` 9 ／ `envelope-broken` 5 ／ `nested-json` 2 ／
`patch-missing` 1 ／ `fence-unbalanced` 1 ／ `over-edit` 1 ／ `no-changes` 1 ／ `unrecognized-format` 1。
**逐語コピーの失敗（apply）と JSON の封筒の崩れ（envelope）で、失敗21件のうち17件を占める。**
開発環境で機序として指し示していた2つが、そのまま実機の主因だった。

**成立率よりも intent の差のほうが大きい。** `current` は「成立」15件のうち8件で改訂が入っておらず、
多くは**古い行が残ったまま新しい行が足される**形だった。出来上がりは正しい Markdown なので
`health` を素通りし、変更行数も1行なので `over-edit` にも掛からない。
→ この壊れ方を捕まえる判定（行数の一致）を足した。**成立率だけを見ていると気づけない**という、
このロードマップ全体の主張がそのまま実例になった。

**構造化出力は救わなかった。** `json_schema` を付けても `current` は 6/12（50%）で、付けない場合
（7/12・58%）と変わらない。`udiff-json` は 4/12（33%）。**①は土台にならない**という P02 の見立ては
実機でも裏付けられた。なおフェンス包みは 2/12 → 0/12 に消えたので、**このサーバーは
`response_format` を黙って捨てていない**（効いてはいる。効いても成立率は上がらない、が結論）。

**判断**: `linenum` を採る。成立率で 97% 対 42%、改訂が入った割合で 97% 対 19%、出力量で1/3。
3つの指標がすべて同じ向きを指している。

**測定中に見つかったベンチ自身の不具合2件**（利用者が発見・修正）:
起動判定が `file://${process.argv[1]}` で **Windows では絶対に一致しない**（`main()` が呼ばれないまま
終了コード0で終わるので、画面に何も出ないのに成功に見える）。`json_schema` の枠が本番テンプレートの
要求欄（`termSuggestions` / `warnings`）と食い違い、矛盾した指示でモデルが封筒ごと文字列に押し込んでいた。
