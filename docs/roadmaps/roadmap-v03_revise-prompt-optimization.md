...
**→ どちらも決着した。形式は `linenum`（ADR-260903-01）、フォールバックの段は作らない（ADR-260908-05）。**
...
```mermaid
graph TD
P01[P01: 測る器を作る<br/>完了] --> P02[P02: 一次選抜と二次選抜<br/>完了]
P02 --> P03[P03: 勝った形式を本番へ入れ、回帰で守る<br/>完了]
```
...
- [x] 改訂の往復を録音し、`lab regress` に足す（実枠5往復・haiku。**指示文を1文変えると落ちることを実測で確認**）
- [x] P02 で「当たったが壊れた」が実際に見つかっていれば、根本修正と単体回帰テストを `src/test/unit/` に固定する（`line-number-residue`。ADR-260908-05）
- [x] フォールバックの段（より簡単な形式で1回だけ聞き直す）を、P02 の数字が要求するときだけ足す → **数字は要求しなかった。作らない**
- [x] `docs/design/prompt.md` の `trans.revisePatch` の節を更新する

**Gates**
- [x] `npm test` と `lab sweep` と `lab regress` がすべて緑（2230 + 52 passing / sweep OK 74・FAIL 0 / regress 新規・改訂とも再生）
- [x] 録音を録り直した場合、**何が変わったのかを説明できる**（黙って上書きしない）→ **録り直していない**。送る指示文を1文字も変えていないため
- [x] 全文再翻訳への自動落下を足していない（手修正を消す道を作らない）

## Gates

- [x] 改訂の成立率が、勝った形式で `current` を上回ることを数字で示せる（42%→97% / 39%→100%）
- [x] 同じベンチが `--base-url` で手元の llama.cpp を指して動く
- [x] 改訂の往復が `lab regress` に入り、指示文を1文字変えれば CI が落ちる（実測で確認 — 下記）
- [x] 訳文を守る仕組み（パッチ失敗時の据え置き）を弱めていない
...
### 仕上げ（P03）で決めたこと・確かめたこと（2026-09-08）

**「当たったが壊れた」の根本修正。** P02 で見つかった `linenum` 固有の穴のうち、指示文で塞げない
ほうを当てはめ器で断つようにした（`line-number-residue`・ADR-260908-05）。**当てはめ器から見れば
正しいパッチ**なので、放っておくと `12<TAB>本文` という行が訳文として保存される。判定は位置まで
見る — 本文の全行が `数字＋タブ` で始まり、その数字が1ずつ増え、先頭の数字がその指示の指す場所と
一致するときだけ。`.tsv` も訳す対象なので、「数字とタブで始まる」だけを根拠にはできない。
AI の答えを検証する段でも同じ判定を通し、そこで気づけたときは同じ形式のまま聞き直す。
回帰は `line-number-patch.test.ts`（7件・巻き込まないことの確認3件を含む）と
`response-validator.test.ts`（5件）に固定した。

**フォールバックの段は作らない。** P02 の数字が要求しなかった。行番号方式の成立率は
97%（qwen3.6-35B-A3B）/ 100%（gemma-4-E4B）で、残る取りこぼしは**同じ形式のやり直し**で足りる
（やり直しの補足はすでに形式ごとに揃えてある）。形式を増やすと、どちらで読むかを推測する必要が
生まれ、「読めてしまうが正しくない」を招く — ADR-260903-01 で避けた当のものである。

**ベンチが本番の当てはめ器を測っていなかった。** `linenum` だけ自前の写しを当てていて、本番へ
入った（ADR-260903-01）あともそれが残っていた。写しの側は `overlapping-ops` を持たず
`INSERT AFTER 0` を範囲外として弾いており、**いちばん大事な候補でこのベンチの前提
（偽物を1つも挟まない）が破れていた**。本番の `applyLineNumberPatch` に寄せた。

**「指示文を1文字変えれば CI が落ちる」を実測した。** `DEFAULT_TRANS_REVISE_PATCH` の
`as shown` を `as given` に変えて `npm run test:byok:e2e` を走らせると、**新規翻訳の再生は通り、
改訂だけが「当たった件数 0」で落ちた**。変更は戻してある。録音は録り直していない。
