...
P02 --> P03[P03: 未カバー経路へシナリオを広げる<br/>完了]
```
...
- [x] 実 UI でしか見えないもの（翻訳中のツリーの遷移、CodeLens、確認ダイアログ）を code-server ホストで撮る
      — `lab ux`（U1〜U5）。**撮るだけでなく文字にも落とす**（`ux.json`）ので、前回との差が取れる

**Gates**
- [x] 上記の各経路に、少なくとも1つの決定的なシナリオがある
- [x] 見つかった不具合は「根本修正 ＋ 単体回帰テスト ＋ シナリオへのアサーション追加」で閉じている
...
- 統合の実走で**もう1つ製品側の疑いが出た**。`src/commands/trans/trans-command.ts:716` は
  ...
  条件（`from` の有無）と意図（改訂かどうか）がずれている。**ADR-260824-02 として直した** —
  実測で user メッセージが 49.8% 減り、参考文と本文でコードブロックの姿が食い違う状態も解消した。
...
- **耐性検査で、翻訳系だけが壊れることが分かった（ADR-260824-01 として直した）。** 同じ意地悪（途中で切れた応答・空の応答・
  ...
  ない。用語・TM 側と同じ「書かずに件数0で終える」へ揃えた。あわせて `finish_reason: "length"` の検知
  （型に定義はあったが一度も読んでいなかった）と、JSON ファイルを訳すときの偽陽性も直した。
- **取り込み（adopt）の後始末に2つ足りないものがある（別枠の候補・着手判断は保留）。** 調査で確定した事実:
  取り込みは対応付けを**その場で訳文の印に書き込んでおり**（`from:<原文のハッシュ>`）、取り込んだ章は
  そもそも翻訳の対象にならない（`need:review` は `needsTranslation()` に含まれない。実測で
  `outcome: "nothing-to-do"`）。つまり「既訳を参考として送る」必要は取り込み経由では発生しない。
  足りないのは次の2つ。
  (1) **ずれた対応を人が直す道が無い。** 対応付けは位置で並べるだけなので、原文にだけ章が1つあると
  以降が玉突きでずれる。`align`（AI による並べ直し）で直せるが、ずれたまま固定されると次の sync の
  「`from` 一致」が維持し続け、自然には直らない（復旧は手作業の手順のみ）。
  (2) ~~**レビュー待ちの既訳が、原文改訂で AI の書き換え対象に変わる。**~~
  **解消（2026-08-31・ADR-260831-02）**: 意図的ではなく、守りの書き漏らしだった。`need:isolate` と
  `need:verify-deletion` は呼び出し元（`updateSectionHashes`）が個別に手当てしているが、
  `need:review` にはその手当てが無く、`from` が付いているため「翻訳済み」の枝へ落ちていた。
  影響は adopt に限らず、`need:review` を付ける6経路すべてに及んでいた（翻訳の品質チェック由来を含む）。
  `need` だけでなく `from` ごと凍結する形で直した — `need` だけ守ると、人が確認を終えて印を外した
  ときに改訂の起点が失われ、改訂の要求そのものが消えるため。frontmatter 側にも同じ分岐が書き写されて
  おり、そちらには「原文を戻したときの後始末」も入っていなかった（もう存在しない原文を指す `revise@`
  が永久に残る）。2つの分岐を1つの実装へ寄せて両方を閉じた。
- **実 UI の段取り（`lab ux`）で分かったこと。** U1〜U5 の観察は run ディレクトリの `ux.json` に
  文字で残る（画像だけでは前回と比べられないため）。実測: (1) AI を使うフォルダ翻訳の前には
  **確認が2枚**立ちはだかる（フォルダ翻訳の確認 `[No / Cancel / Yes]` → AI 初回利用の説明
  `[Cancel / Proceed]`）。後者は headless では `MDAIT_DEBUG_IPC=1` で飛ばしているので実ホストでしか見えない。
  (2) フォルダ翻訳の確認は **`message` が空で、文言はすべて `detail`** に入る。
  (3) 翻訳中は**ファイルの行まで回転アイコン**が出て、まとめの行の数字も `child (0/9)` → `(1/9)` と進み、
  終わると止まる（ADR-260803-01 の「下ろし忘れ」は起きていない）。
  (4) CodeLens の品揃えは訳文側が `✨Translate / ✓Mark as Translated / ⎘Source / ⋮More`、
  原文側が `⎘Target / ⋮More`。
- **道具の側で2つ直した（どちらも実 UI を見るときだけ出る）。** Playwright の `innerText()` は
  無い要素を既定の30秒待ち続けるため、補足（detail）の無いダイアログを1回読むのに30秒かかり、
  **その間ほかの操作が全部止まっていた**（実測: `dialog-policy` が60秒応答しない）。1回の
  `evaluateAll` で読み切る形へ直した。もう1つは折り畳みの開き方で、上から開くと深さ方向へ潜り続け
  隣の枝に永久に届かない（実測: 91回押しても `en` が畳まれたままだった）。下から開く形へ直した。
- **実 UI でしか出ない製品側の欠陥を1件見つけて直した（ADR-260824-03）。** デバッグ IPC に相対パスを
