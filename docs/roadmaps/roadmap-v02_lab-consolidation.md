...
- **取り込み（adopt）の後始末に2つ足りないものがある（別枠の候補・着手判断は保留）。** 調査で確定した事実:
  ...
  (2) **レビュー待ちの既訳が、原文改訂で AI の書き換え対象に変わる。** `marker-sync.ts:225-245` の
  分岐順序で `setReviseNeed` が先に当たり、`need:review` が `need:revise` に置き換わる。人の確認が
  済んでいない訳文が確認を経ずに改訂へ進む。意図的かどうかは ADR にも設計文書にも書かれていない。
