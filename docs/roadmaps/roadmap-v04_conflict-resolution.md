...
「競合の解決」の枝に、衝突している用語・TM のエントリ・`unit-state` の行が1件1行で並ぶ。`競合を解決`
を押すと、何件が決まり何件が残るかの確認が1枚出て、承認すると鍵の突き合わせで決まる分が書き戻る。
結果は `.mdait/local/reports/` のレポートに残る。決まらなかった件はツリーに残り、行内の `あなたを残す` /
`相手を残す` で人が決める。

> **AI は使わない**（2026-09-12 の決定。ADR-260912-04）。P02 では ✨`競合を AI で解決` を主導線に
> 据えたが、実測で分かったのは「AI へ回る件がそもそも稀で、回るときには材料が無い」ことだった。
> 以下の P02・P03 は当時の計画のまま残す（経緯として読む）。
...
- ~~[TODO] 実行時見直し：LM Tool（`mdait_getStatus`）は競合の件数を返さない。エージェントが競合に気づけないままでよいか~~
  → **返すようにした**（2026-09-23・ADR-260923-05）。`data.conflicts` と要約・`nextActions` に出す。解決の手段は渡さず、人に頼むよう案内する
...
コストの事前提示なので、判定が終わってから確認を出す形にはしない。結果は `.mdait/local/reports/` へ書いて
...
- [x] 録音の再生（`npm run test:byok:e2e`）に競合解決の往復を足した（`conflict-resolve-terms.jsonl`。実機は綴りの誤りを見抜いて正しい側を選んだ）→ AI の判定ごと廃止したので、この録音も消した（ADR-260912-04）
...
- **判定に添える材料は、競合していないファイルからしか採れない。** 用語集が競合しているときの用語集も、TM が競合しているときの TM も、その場では読めない。結果として「TM の競合には用語集が付き、用語集の競合には TM が付く」形になる（`conflict-evidence.ts`）。原文の旧版と新版は、TM と用語集の競合には結び付く相手が無いので渡していない
- **「片方が消し、片方が直した」は AI に決めさせられない。** 消した側には見せる値が無く、祖先の値を置くと削除が黙って取り消される。印を持って人へ回す（ADR-260912-03）
- ~~[TODO] 実行時見直し：`JUDGE_BATCH_SIZE`~~ → **判定ごとやめた**（2026-09-12。ADR-260912-04）
...
StatusTree の行内に `あなたを残す` / `相手を残す` を置く。Hover には両側の全文を出す。
AI を呼ばないので ✨ は付けない。
...
- [x] ✨ が付いていない操作が AI を呼ばない（`あなたを残す` / `相手を残す` は AI を1回も通らない）
...
- ~~**frontmatter のキーごと分割**（`docs/design/merge-resilience.md`「残っている対策案」1）は、このロードマップの
  あとに回した（2026-09-11 の判断）。~~ → **採らない**（2026-09-23・ADR-260923-04）。`title` と
  `description` を別々に直した2人を合流させても、競合は解決と次の同期で片付き、原稿は1文字も
  変わらず、両方の直しが入った frontmatter は改訂待ちになる（`frontmatter-merge.test.ts`）。
  キーごとに分けて得られるのは「改訂待ちにならずに済む」ことだけで、形式変更の代償に見合わない
...
- `1 TU = 1 行`（ADR-260908-01）は union のための工夫だったが残す。行単位のほうが競合の範囲が小さく、
  両側を切り出しやすい。
- ~~[TODO] 実行時見直し：AI の判定語彙~~ → **判定ごとやめた**（2026-09-12。ADR-260912-04）
- ~~[TODO] 実行時見直し：合流由来の `held` 行を同じ枝に混ぜたときの件数。実運用で多すぎるなら別の枝へ分ける~~
  → **同じ枝のままでよい**（2026-09-23）。合流由来の行は、そのファイルの次の同期で必ず競合の数から外れる
  （一致した側は席へ戻り、どちらとも一致しなければ印を外して預かりへ移す。ADR-260923-04）。溜まるのは
  「合流してから、まだ同期していないファイル」の分だけで、`autoSyncOnSave` か明示 sync で消える。
  調べる途中で、**どちらとも一致しない行が競合として永久に残る**欠陥が見つかり、あわせて直した
  （ツリーの行に操作が無く、解く手段が無かった。frontmatter の行も同じだった）
...
実装の順序について2つ決めた。`.gitattributes` の撤去は
`unit-state` の合流の姿を変えるので、実測台を先に直す。そして frontmatter のキーごと分割と
触る場所が重なるので、順序を先に決める。
