...
`terms.csv` は跡を1つも残さず（`terms-repository-csv.ts` の畳み込み）、`translations.tmx` はログに1行だけ
...
を1回押すと、AI が**原文の旧版と新版・`from` の対応・TM・用語集**を材料にどちらを採るかを決め、
「5件をこう解きます」の確認を1枚出してから書き戻す。結果は `.mdait/reports/` のレポートに残る。
AI が迷った件はツリーに残り、行内の `こちらを採る` / `あちらを採る` で人が決める。
...
代わりに払う代償がある。union が正しく処理していた「2人が別々のエントリを足しただけ」も競合として
人の前に出る（TM の実測で 16/20・18/20 の頻度）。**手間は増えるが、黙って消えることは無くなる。**
...
対象は `unit-state`・`unit-registry`・`translations.tmx`・`terms.csv` の4つ。判定は
`hasConflictMarkersInDataFile` を使う（原稿用の `hasConflictMarkers` はコードブロックを見るので、
`.mdait` の中のファイルには当てない）。あわせて `unit-state` の `held` 行も未解決として数える — これは
競合マーカーとは別の入口で、読み込みが既に両側を拾って席から降ろした行である（`seatOnLoad`）。
...
**Steps**
- [ ] `mdait-dir.ts` の `.gitattributes` 自動生成をやめる。既存の作業場では mdait が書いた行だけを消し、他人が書いた行は残す。ファイルが空になったら消す
- [ ] `.mdait` の4ファイルの未解決を数える係を作る（`unit-state` の `held` を含む）
...
- [ ] union 無しの実測で消失 0（読み込みが両側を拾うため）。競合の件数を数字として記録した
...
✨`競合を AI で解決` コマンドを作る。確認ダイアログ1枚（件数・何を書くか・取り消せるか）を出してから
実行し、結果を `.mdait/reports/` へ書いて通知のボタンから開く（`report-file.ts` 経由）。骨格は
`commands/ai-review/` と揃える。
...
書き戻しは既存の入口を通す。`unit-state` は `commands/markers/unit-mutation.ts`（embedded ではここで
原稿のマーカー行も直る）、TM は `TmxStore`、用語集は `TermsRepositoryCSV`。新しい書き込み経路は作らない。

AI が迷った件と、API キーが無い場合は**書かずに残す**。残った件数はレポートとツリーに出る。

**Steps**
- [ ] 競合マーカーの入ったファイルから両側と共通の祖先を切り出す係
- [ ] 判定の語彙と応答の形式を決める（`ai-review` の `verdict` とは別物。`merged` を許すかは実行時に決める）
- [ ] 指示文（`src/prompts/`）と応答の検証
- [ ] ✨コマンド・確認ダイアログ・レポート・通知ボタン
- [ ] 書き戻しを4種の対象それぞれの入口へ繋ぐ
- [ ] l10n

**Gates**
- [ ] 定常 sync（`autoSyncOnSave` 含む）は1行も AI を呼ばない（ADR-260705-01）
- [ ] 確認ダイアログを閉じたら1バイトも書かれない
...
`held` 行の照合もこの段に入れる。これは訳の良し悪しではなく事実の照合で、原稿のハッシュと突き合わせれば
決定的に決まる — 一致する側を席へ戻し、一致しなければ人に選ばせる。
...
`.gitattributes` と溜まった `held` 行があり、そこを通る道も測る対象である。
...
`held` 行の解消）を確認する。`docs/design/merge-resilience.md` の数字を、union 無しの実測で置き換える。
...
- [ ] 黙って片方が消える経路が1つも残っていない（`terms.csv` の先勝ち畳み、`translations.tmx` のログだけ、の両方が無くなった）
...
- `1 TU = 1 行`（ADR-260908-01）は union のための工夫だったが残す。行単位のほうが競合の範囲が小さく、
  両側を切り出しやすい。
- [TODO] 実行時見直し：AI の判定語彙と、`merged`（両方を合わせた新しい値）を許すかどうか
- [TODO] 実行時見直し：`held` 行を同じ枝に混ぜたときの件数。実運用で多すぎるなら別の枝へ分ける
