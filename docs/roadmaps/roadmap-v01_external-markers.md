...
関所（敵対的レビュー）は1周し、4件を直して閉じた。そこから送られた項目のうち P05b に
...
関所をここに置いていた。2026-08-16 に1周し、行の種別の数え間違い・frontmatter だけ訳した
...
- ~~技術債（このロードマップの外）: `SectionMatcher` の `interval-align` への載せ替え（§10）が残る。~~
  **解消（2026-09-23・ADR-260923-03）**: 錨は `from` の一意一致、区間は順序の保たれる錨だけで切る形に
  揃えた。旧実装と乱択入力2万件で突き合わせ、取りこぼし0件。途中で、相手のいない訳文（人が書き足した章・
  原文を失った訳文）が sync のたびにファイル末尾へ動く欠陥が見つかり、あわせて直した。
...
- **敵対的レビューから技術債へ移した4件（2026-08-22 の選別。既定を倒しても踏む確率が変わらないもの）**:
  ...
  - ~~段階4（内容による再リンク）の順序埋めに縮みのガードが無い。~~
    **欠陥ではなかった（2026-09-23）**: 結び直しは被覆率 0.7 を両方向に要求するので、ユニット数が
    3割より多く減った訳文（パースの崩れで潰れた訳文）とはそもそも結び直さない。3割以内の減り方で
    結び直したあとの刈り取りは、動いていないファイルと同じ `shouldPruneLeftovers` を通る。被覆率の
    下限が縮みの守りを兼ねていることを単体テストで固定した（`content-relink.test.ts`）。
  - ~~`parkEntriesFrom` に「預ける値打ち」の判定が無く、保留席が単調に増える。~~
    **解消済み（確認 2026-09-23）**: `parkEntriesFrom` はもう無く、預け入れは `parkEntries` 1つに
    まとまった。預ける値打ちの判定は読み込み時の控え（`buildAlignmentMemo` の `hasStateWorthHolding`）に
    入っている。ユニットが急に減ったときの守り（`shouldPruneLeftovers` が偽の経路）だけは値打ちで
    絞らず全部預ける — 原文の行は hash しか持たないが、訳文の `from` が指す先なので、崩れが直ったとき
    完全に戻すには失えない（probe S69）。同じ本文 hash の預かりは1つに絞られるので、増えるのは
    「消えたきり戻ってこなかった、内容の異なる章」の数だけである。
  - ~~大文字小文字だけのリネーム（`README.md` → `Readme.md`）への追随。~~
    **解消（2026-09-23・ADR-260923-06）**: 綴りで判定するようにした。調べる途中で、訳文を連れて
    動かす編集の `ignoreIfExists` が `overwrite: false` と並べてあったせいで一度も効いていなかった
    ことが分かり、あわせて直した（VS Code は `overwrite` が未指定のときだけ `ignoreIfExists` を見る）。
...
| # | 問い | 選んだ案 | 理由 |
|---|---|---|---|
| 1 | 敵対的レビューが送り出した積み残し6件をどう扱うか | 「**既定を倒したせいで初めて踏むか**」で選別し、該当するものだけ関所に残す | 全部直す案は関所の役目を取り違えている。関所の目的は「新規ユーザーの既定の経路に欠陥を置かない」ことであって「external の宿題を全部消す」ことではない。含めると関所が膨らんで永遠に開かない。先に倒して踏まれたら直す案は、mdait の壊れ方が「人の訳が黙って `need:translate` に落ちる」種類なので採れない |
