...
原文が更新された際、mdaitは前回の原文をスナップショットファイルから参照して、変更情報をAIプロンプトに含めます。

- **保存タイミング**: Sync実行時にすべてのユニットのスナップショットを保存
- **ストレージ**: `.mdait/snapshot`ファイルに圧縮（gzip + base64）
- **ガベージコレクション**: Sync時に不要なスナップショットを自動削除
