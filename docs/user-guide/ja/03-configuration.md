...
| パラメータ | 型 | デフォルト | 説明 |
|-----------|-----|----------|------|
| enabled | boolean | true | TM機能の有効/無効 |
| maxReferences | number | 5 | 翻訳時にAIに提供するTM参照の最大数（1-20） |

TMを無効にした場合、tm-commitコマンドとtrans実行時のTM参照の両方が無効になります。

## fix（ユニット確定設定）

ユニット確定（fix）コマンドの動作を設定します。
...
| パラメータ | 型 | デフォルト | 説明 |
|-----------|-----|----------|------|
| tm | boolean | false | fix実行時にTM登録も同時に行うかどうか |

`tm: true`に設定すると、ユニット確定時に自動的にTM登録が実行され、確定済みユニットが翻訳メモリに蓄積されます。なお、この設定が有効に機能するには`tm.enabled: true`である必要があります。
