...
| `sync.orphanTargetPolicy` | 文字列 | `"delete"` | 孤立ユニットの処理（`delete`/`verify`/`keep`） |
...
| `orphanTargetPolicy` | 文字列 | `"delete"` | 孤立ユニットの処理ポリシー: `"delete"`（自動削除）/ `"verify"`（`need:verify-deletion` 付与）/ `"keep"`（`need:keep` で恒久保持）。`autoDelete` より優先 |
