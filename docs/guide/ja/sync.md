...
**独立ユニット**: `from` のないマーカー（例: `<!-- mdait a1b2c3d4 -->`）を持つ訳文側ユニットは「独立ユニット」として扱われます。原文に対応相手を持たない訳文側の独自コンテンツで、sync は対応付け・削除の対象にせず常に保持し、trans も翻訳しません（翻訳率の分母からも除外）。`need:isolate` のユニットも同様に保持されます。詳細は設計文書 [command_sync.md](../../design/command_sync.md) の「孤立ユニットモデル」を参照してください。
...
```json
"sync": {
  "orphanTargetPolicy": "verify"
}
```

| `orphanTargetPolicy` | 動作 |
|---|---|
