...
| **tm** | 翻訳ユニットをTMX（翻訳メモリ交換フォーマット）へ文単位登録 | [command_tm-commit.md](design/command_tm-commit.md) |
...
#### P5: 層の責務分離
Core層はVS Code APIに非依存で各層が明確な責務を持ち、下位層は上位層に依存しない。独立したテスト・交換が可能（詳細：[アーキテクチャの全体像](#アーキテクチャの全体像)）。
