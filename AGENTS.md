...
- 管理下 Markdown の書き出しは `infra/workspace/managed-write.ts` の `writeManagedMarkdown` / `writeManagedMarkdownSync` だけを通す。原稿の改行コードと末尾改行を測って揃え直し、出来上がりが1バイトも違わなければ書かない処理はそこにしか無い。素の `writeFile` で書くと、Windows で書かれた原稿が全行 LF に書き換わる（実測。内容は同じなのにファイル全体が差分になる）。`.mdait/` の中の管理ファイルは原稿ではないので対象外（ADR-260902-01）
