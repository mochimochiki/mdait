...
- **`src/lm-tools/`** — Copilot Chat 向け LanguageModelTool API ラッパー（`mdait_getStatus` / `mdait_sync` / `mdait_translate` / `mdait_term` / `mdait_tm` / `mdait_validate` / `mdait_aiReview` / `mdait_adopt` / `mdait_resolve` の9ツール。共通JSONエンベロープ）。
