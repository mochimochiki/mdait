...
関連付けられたMarkdownファイル群内のmdaitマーカーで定義されるユニット単位で、ハッシュ・`src`追跡・`need`フラグの同期を行う。
...
- 多段でも `src` は常に翻訳元ユニットのハッシュを指し、構造をシンプルに保つ。翻訳の方向が混在（ja->en,en->ja)する可能性があるが、`sync`により変更があった原稿を新たな`src`として各言語への伝搬の自動付与を行うことで、翻訳元の追跡が可能。
