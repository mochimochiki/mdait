...
- Code blocks dropped by the AI during translation are now reported as a warning instead of vanishing silently, and non-Markdown files (`trans.extensions`) flag `need:review` when that happens — but only for that, so translating a `.json` file no longer asks for review every time
