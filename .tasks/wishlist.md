...
**背景・目的**
debug-ipc基盤とコマンド結果の構造化（→チケット`260328_デバッグ可観測性改善`）が整った後、エージェントがEnd-to-Endシナリオを自律的に実行・検証できる仕組みを設計する。

**コンセプト**
Skillにシナリオを定義し、エージェントがIPC経由で`sync → trans → tm.commit → ソース変更 → re-sync → patchMode trans`の改訂フローまでを自動実行・アサーションする。

**前提**
- `260328_デバッグ可観測性改善`チケットの完了（sync/transの構造化result）

**TODO**
- 改訂フローのEnd-to-Endシナリオ定義
- TM参照がプロンプトに渡った効果の検証（tm.commit後に別ファイルのtransでTM hits > 0になるか確認）
- term.detect / term.expand のIPCテスト実行
- エージェント自律テストのSkill設計

**想定問答**
- Q: vitestのGUIテストとの住み分けは？
  A: vitestはユニット〜モジュール単位。自律テストはIPC経由のEnd-to-Endフローで、実際のExtension Host上で動く統合シナリオ。
