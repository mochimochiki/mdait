...
**改訂（revise）の出力形式を比べるなら `lab bench-revise` を使う。** あちらは `agent` の受け皿を自分で立て、
ケース × 候補を総当たりして「どの段で落ちたか」を数える。指示文を読み比べるのではなく、成立率で選ぶための道具。
判定の筋道を確かめるだけなら `--self-test` で LLM を1回も呼ばない。
