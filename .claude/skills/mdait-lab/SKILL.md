...
| `site` | 規模のある見本サイト（対訳49ファイル）を書き出す。`--out`（既定 `/tmp/mdait-site`）・`--markers embedded\|external`・`--extra-langs ko`（対象言語を足す） |
| `hugo` | 見本サイトを Hugo で**建てる**。`--save`（指紋を残す）・`--baseline`（残した指紋と比べる）。Hugo は PATH か `MDAIT_HUGO_BIN`。無ければ素通り |
...
「47ファイル」と書いてある行は、**見本サイトがまだ47ファイルだった頃の実測**である。その後、
書式のバリエーション（箇条書き・引用・脚注・生 HTML・ショートコード・空の章・frontmatter の型）と
章の索引ページを足して49ファイルになった。往復数と所要時間はそのぶん増える。
...
| **対象言語を足すと、初回から2本とも対象になる**（`--extra-langs ko`・決定的な経路の実測）。対象ファイル 25 → 50、取り込み済み 84 → 105、新規 8 → 67 ユニット。訳文の無かった 17 ページと非Markdown 3本が `need:translate` で作られ、ko が原文と同じ 25 ファイルに揃う。3言語のまま Hugo も建つ（HTML 98 本） | ADR-260825-01 の振る舞いを、**揃い方の違う2本の対**で確かめた形。**実 LLM の費用は対の本数に比例する**ので、既定は1対のまま |
| **翻訳メモリも実際に訳文へ効く**（実 LLM・haiku・同じ原文4ファイルを TM あり／なしで訳し分けた実測）。仕込んだ言い回し9件が**すべて**訳文に出た。**近似一致（原文と少し違う文）も carry over する** — 実運用で完全一致は稀なので、ここが効かなければ TM は役に立たない | 用語集と同じ A/B の作り。TMX は `.mdait/translations.tmx` に手で置ける（形は `docs/adr.md` の ADR-260905-01 と `tmx-store.ts`）|
| **矛盾する参考を2件渡すと、AI はどちらも採らず自前で訳す。** 同じ原文に別の訳を入れた群では、訳1（Out of the box / teammate）も訳2（squad members）も出ず、訳2の構造だけ借りた別の訳になった | 混ざりはしないが**一貫性は失われる**。TM を入れた意味が消える。重みや新しさで1件に絞る余地がある（未着手）|
| **参考が届いても採用されるとは限らない。** 近似一致が効かなかった走行があり、ログでは `TM references found {"count":1}` と**同じ参考がちゃんと届いていた**。引き当ての問題ではなく採用の揺れ | 「効かない＝引き当てが悪い」と決めつけない。まずログで届いたかを見る。**揺れの大きさは未測定**（各群1回ずつしか回していない）|
| 用語集は規則の中で "Follow the provided terminology list **strictly**"、TM は "Use them as reference for consistency, **but prioritize accuracy and context**" | 強さの差は**設計どおり**。実測の挙動はこの指示に完全に一致する |
| **用語集は実際に訳文へ効く**（実 LLM・haiku・同じ原文4ファイルを用語集あり／なしで訳し分けた実測）。わざと「AI が自然には選ばない訳語」を4語入れたところ、**全部が反映された**。`ノート→memo`（note 18回 → memo 14回）・`タグ→label`（tag 18回 → label 16回）・`共有→distribution`（sharing 5回 → 0回）・`クモノート→CloudNote` | 用語集の存在理由そのものの実測。**用語集が無いと製品名は揺れる** — 同じ走行の中で題名が "Kumonote"、本文が "Kumo Note" になった |
| 用語集が効かなかったのは2か所だけで、**どちらも訳してはいけない場所**だった。`[Working with memos](notes/)` のリンクのパスと、`` `#"meeting notes"` `` のインラインコードの中 | 「残っている＝効いていない」ではない。**インラインコードが残るのは正しい**（用語照合はコードを除外する） |
| **frontmatter の題名にも効く**（`Term references found for frontmatter` が title・description の両方で出る）。"Kumonote Document" → "CloudNote Document"、"Organize by tags" → "Organize with Labels" | frontmatter へ用語集を渡す修正の実地確認。直す前は題名だけ用語集の外にあった |
| 用語集は**助言であって置換ではない**。`共有→distribution`（名詞）を指定した題名が "distribute"（動詞）になった | 機械的に置き換えていない証拠でもあるが、題名が小文字始まりの動詞になるなど、見栄えは AI の判断に委ねられる |
| 4ファイル12ユニット＋frontmatter 6件の翻訳が **295秒（用語集なし）／348秒（あり）** | 用語集の有無で所要は大きく変わらない。プロンプトに載るのは当たった用語だけなので、費用も増えない |
| **翻訳後のサイトは実際に建つ**（Hugo 0.148・見本49ファイル）。取り込みの前後で建てて出力を比べると、**原文側の HTML は1本も変わらない**。変わるのは訳文側だけで、内訳は「訳文に無かった章が原文のまま足された」11本と、新しく作られた訳文2本 | 「原稿が1バイトも変わらない」の一段先。frontmatter の型が崩れればビルドがその場で失敗し、本文の構造が崩れれば HTML が変わる。`lab hugo --save` → 取り込み → `lab hugo --baseline` |
| **embedded のマーカーは公開される出力に出る。** RSS（`index.xml`）に68か所そのまま残り、`--minify` を付けない HTML にも出る。**external では1件も出ない**（唯一の一致は、コードブロックにわざと仕込んだ囮そのもの＝正しい振る舞い） | 公開するサイトを持つ人には external を勧める材料。`--minify` は HTML のコメントだけを落とし、RSS の `<description>` はエスケープ済みなので落ちない（ADR-260904-01） |
| **見本のホームは `_index.md` でなければならない。** `index.md` だと静的サイトジェネレータが**サイト全体を1ページの付属物として畳む**（出力 HTML が65本のはずが7本になった） | 見本サイトの作りを実サイトに寄せる話。mdait 側の欠陥ではない |
...
| **改訂を実 LLM・規模で通すと当たる**（取り込み済みの見本サイトの原文9本を書き換え、本文10ユニット＋frontmatter 1件がすべて反映。245秒・20ユニット）。表・箇条書き・コードブロック・CRLF・frontmatter のどれも壊れなかった | ADR-260903-01 の `linenum` 形式が、**複数ファイル・複数パッチ**でも成立することの実測。壊れやすい形は個別に確かめる（表の行数・``` の数・改行コード） |
| **同じ走行で製品の欠陥が2件出た。** AI の用語候補に `source` が無いとファイル1本の翻訳ごと落ちる（ADR-260903-06）、frontmatter の改訂で引用符が落ちる（ADR-260903-07） | **どちらも echo では出ない** — 偽の AI は用語候補を返さず、frontmatter の改訂も起こさない。実 LLM を通す価値がここにある |
| 取り込みのあとに確認待ちで残るのは5件だけ（非MD 3本＋訳文だけにある章2件）。どちらも設計どおり（パターン10・3）。**うち非MD 3本は ADR-260903-08 でレビュー対象になったので、次に測ると減るはず（未実測）** | 「残っている＝失敗」ではない。件数の内訳を見る |
