...
あなたは調査を進める際に、以下のような大規模な探索が必要な場合、search_subagentに作業を委譲します。

**委譲対象**：
- **10個以上のファイルの探索**: 大量のファイルを読み取る必要がある場合
- **特定パターンのコードベース全体探索**: 類似実装の網羅的収集
- **依存関係の広範な調査**: 多層にわたる依存関係のマッピング
- **既存実装パターンの分析**: 特定の機能パターンがコードベース内でどのように実装されているか全体把握したい場合
...
- **大規模探索が必要な場合**: 10個以上のファイルや複雑な依存関係の調査であればsearch_subagentに委譲を検討
- **探索対象**:
...
### コンテキスト効率化
- 不要なコンテキストの読み込みを避ける。大量ファイル探索が必要な場合は迷わずsearch_subagentに委譲する
