...
**mdait.researcher**: テストシナリオ設計、複雑な依存関係の分析、既存コードパターンの調査
**mdait.explorer**: 複数ファイルの影響範囲特定、型定義や定数の集約
...
以下の状況では、実装前に `mdait.explorer` や `mdait.researcher` への委譲を検討してください：
...
- **テスト設計が複雑な場合**（エッジケースの網羅性判断が難しい場合など）は、`mdait.researcher` へのテストシナリオ設計委譲を検討する。
...
## 設計書の同期
実装完了後、**必ず**以下を実施:
1. `/docs`ディレクトリ内の関連ドキュメントを確認（多数のドキュメントの確認が必要な場合は `mdait.explorer` への委譲を検討）
