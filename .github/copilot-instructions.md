...
- 正規表現を使用する場合、手動パースではなく可能な限り既存のユーティリティ関数を使用する。手動パースが必要な場合も、まずはメソッド・クラスなどに切り出して保守性を高められないか立ち止まって検討する。
...
- **日本語で**レビューを行う。
- 中長期的な保守性、設計の一貫性、整合性、拡張性に重点を置く。
- セキュリティ的に重大な問題が見つかった場合、必ず報告する。
- `/design` との整合性を確認し、設計意図を理解してレビューする。`/design`側の同期が漏れている場合はそれも指摘する。