...
- 方針・骨子がまとまったら変更や新機能については`design.md`を修正する。要点を簡潔に書くことを心がける。
- `design.md`の詳細設計は、`tasks/do/yymmdd_<作業名>.md`に作業チケットとして記載。yymmdd は今日の日付。ファイル名は日本語で簡潔に記載。
- 作業チケットは簡潔に。おおむね100行以内・1500文字以内で。
- 承認が得られたら実装に進む。実装完了時には`tasks/do`から`tasks/done`にファイルを移し、設計との整合性チェックおよび実装時のメモを記載。
- 作業チケットは以下のフォーマットに従う。
```markdown
# 作業チケット: <作業名>
...
(処理フローの全体像を骨子が伝わるように簡潔に記載。主要フェーズとその処理内容を簡潔に列挙)
